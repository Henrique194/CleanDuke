...
void CONFIG_ReadSetup(void) {
    ...
    if (dummy) {
        if (!VOLUMEONE) {
            ...
            Error(EXIT_SUCCESS, "The -map option does not work with the "
                                "Shareware version of duke3d.grp\n"
                                "Change your duke3d.grp file to the 1.3d "
                                "version or 1.5 Atomic version\n");
        }
    }
    ...
    SCRIPT_GetString(scripthandle, "Screen Setup", "Password",
                     &ud.pwlockout[0]);
    ...
    SCRIPT_GetNumber(scripthandle, "Screen Setup", "ScreenHeight",
                     &ScreenHeight);
    SCRIPT_GetNumber(scripthandle, "Screen Setup", "FullScreen", &FullScreen);
    SCRIPT_GetNumber(scripthandle, "Screen Setup", "ScreenGamma",
                     &ud.brightness);
    SCRIPT_GetNumber(scripthandle, "Screen Setup", "ScreenSize",
                     &ud.screen_size);
    SCRIPT_GetNumber(scripthandle, "Screen Setup", "ExtScreenSize",
    ...
    SCRIPT_GetNumber(scripthandle, "Misc", "ShowCinematics",
                     &ud.showcinematics);
    SCRIPT_GetNumber(scripthandle, "Misc", "WeaponAutoSwitch",
                     &ud.weaponautoswitch);
    ...
    if (ud.auto_aim != 1 && ud.auto_aim != 2)
        ud.auto_aim =
            2; // avoid people missing with the cfg to go in a deadlock
    ...
    SCRIPT_GetNumber(scripthandle, "Sound Setup", "AmbienceToggle",
                     &AmbienceToggle);
    SCRIPT_GetNumber(scripthandle, "Sound Setup", "OpponentSoundToggle",
                     &OpponentSoundToggle);
    ...
    SCRIPT_GetNumber(scripthandle, "Sound Setup", "ReverseStereo",
                     &ReverseStereo);
    SCRIPT_GetNumber(scripthandle, "Controls", "ControllerType",
                     &ControllerType);
    SCRIPT_GetNumber(scripthandle, "Controls", "MouseAimingFlipped",
                     &ud.mouseflip);
    SCRIPT_GetNumber(scripthandle, "Controls", "MouseAiming", &MouseAiming);
    SCRIPT_GetNumber(scripthandle, "Controls", "GameMouseAiming",
                     (int32*) &ps[0].aim_mode);
    SCRIPT_GetNumber(scripthandle, "Controls", "AimingFlag",
                     (int32*) &myaimmode);
