...
#define SDL_CHECK_SUCCESS(val)                                                 \
    __checkSdlSuccess((val), #val, __FILE__, __LINE__)
#define SDL_CHECK_NOT_NULL(val, message)                                       \
    __checkSdlNotNull((val), (message), __FILE__, __LINE__)


inline static void __checkSdlSuccess(
    int result,
    char const* func,
    ...
    if (result != 0) {
        Error(EXIT_FAILURE,
              ...
              "SDL: FILE: %s\n"
              "SDL: LINE: %d",
              func, SDL_GetError(), file, line);
    }
}

inline static void __checkSdlNotNull(
    void* handle,
    char const* msg,
    const char* file, int line
) {
