...
//
// tanguyf: 2020-05-03: when patchstatusbar() is called,
// it can sometimes not clear the entire area, leaving
// ghost pixels from the previous frame at certain resolutions
// (e.g. ammo counter leaving the bottom part of the previous digits).
//
// This does not append when the Y resolution is a
// multiple of 200 (from 320x200) and therefore the division
// has no remainder. Although not all non-multiple-of-200
// resolutions seem to exhibit the problem.
//
// I believe this is caused by division round-down vs round-up,
// and other inconsistencies in the engine (keep in mind that 4K
// resolution did not exist when this engine was created).
// But verifying it in a consistent manner would take quite a while.
//
// This is my attempt at fixing the problem, by making
// patchstatusbar() use round-up division here to always
// clear the boundary pixels in case the division has a
// remainder.
//
...
static inline int tmulscale(int32_t i1, int32_t i2, int32_t i3, int32_t i4,
                            int32_t i5, int32_t i6, int32_t shift) {
...
#define DEFFUN(N)                                                              \
    static inline int32_t dmulscale##N(                                        \
        int32_t input1,                                                        \
        int32_t input2,                                                        \
        int32_t input3,                                                        \
        int32_t input4                                                         \
...
#define DEFFUN(N)                                                              \
    static inline int32_t tmulscale##N(                                        \
        int32_t i1,                                                            \
        int32_t i2,                                                            \
        int32_t i3,                                                            \
        int32_t i4,                                                            \
...
void copybufbyte(void* src, void* dest, i32 count);
