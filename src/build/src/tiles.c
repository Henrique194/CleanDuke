...
void setviewtotile(short tilenume, int32_t tileWidth, int32_t tileHeight) {
    ...
    copybufbyte(&startumost[windowx1], &bakumost[windowx1],
                (windowx2 - windowx1 + 1) * sizeof(bakumost[0]));
    copybufbyte(&startdmost[windowx1], &bakdmost[windowx1],
                (windowx2 - windowx1 + 1) * sizeof(bakdmost[0]));
...
void loadtile(short tilenume) {
    ...
    if (tiles[tilenume].data == NULL) {
        tiles[tilenume].lock = 199;
        allocache(&tiles[tilenume].data, tileFilesize,
                  (uint8_t*) &tiles[tilenume].lock);
    }
...
int loadpics(char* filename, char* gamedir) {
    ...
    do {
        ...
        if ((fil = TCkopen4load(artfilename, 0)) != -1) {
            ...
            for (i = localtilestart; i <= localtileend; i++)
                kread32(fil, &tiles[i].anim_flags);

            offscount = 4 + 4 + 4 + 4 + (
                            (localtileend - localtilestart + 1) << 3);
            for (i = localtilestart; i <= localtileend; i++) {
                tilefilenum[i] = k;
    ...
    int32_t cachesize = max(artsize, 1048576);
...
//
// If a texture is animated, this will return the
// offset to add to tilenum in order to retrieve
...
int animateoffs(int16_t tilenum) {
    ...
    i = (totalclocklock >> ((tiles[tilenum].anim_flags >> 24) & 15));

    if ((tiles[tilenum].anim_flags & 63) > 0) {
        switch (tiles[tilenum].anim_flags & 192) {
            case 64:
                k = (i % ((tiles[tilenum].anim_flags & 63) << 1));
                if (k < (tiles[tilenum].anim_flags & 63))
                    offs = k;
                else
                    offs = (((tiles[tilenum].anim_flags & 63) << 1) - k);
                break;
            case 128:
                offs = (i % ((tiles[tilenum].anim_flags & 63) + 1));
                break;
            case 192:
                offs = -(i % ((tiles[tilenum].anim_flags & 63) + 1));
        }
