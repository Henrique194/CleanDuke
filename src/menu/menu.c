...
static short last_zero;
static short last_fifty;
short last_threehundred = 0;

static uint8_t fileselect = 1;
static uint8_t menunamecnt;
...
static int32_t quittimer = 0;

static int32_t lastanimhack = 0;
...
int loadpheader(uint8_t  spot,int32 *vn,int32 *ln,int32 *psk,int32 *nump)
{
    ...
    int32_t bv;
     ...
     if(bv != BYTEVERSION)
     {
        HU_SetPlayerQuote(114,&ps[myconnectindex],1);
        kclose(fil);
        return 1;
     }
...
int loadplayer(int8_t spot)
{
    ...
    if (nump != numplayers)
    {
        ...
        HU_SetPlayerQuote(124, &ps[myconnectindex], 1);
        return 1;
    }
    ...
    while (k >= 0)
    {
        ...
        {
        case 31:
            P_SetInterpolation(&sector[sprite[k].sectnum].floorz);
            break;
        case 32:
            P_SetInterpolation(&sector[sprite[k].sectnum].ceilingz);
            break;
        case 25:
            P_SetInterpolation(&sector[sprite[k].sectnum].floorz);
            P_SetInterpolation(&sector[sprite[k].sectnum].ceilingz);
    ...
    for (i = animatecnt - 1; i >= 0; i--)
        P_SetInterpolation(animateptr[i]);
...
void dispnames(void)
{
    ...
    HU_DrawMiniText(c,48,ud.savegame[0],2,10+16);
    HU_DrawMiniText(c,48+12,ud.savegame[1],2,10+16);
    HU_DrawMiniText(c,48+12+12,ud.savegame[2],2,10+16);
    HU_DrawMiniText(c,48+12+12+12,ud.savegame[3],2,10+16);
    HU_DrawMiniText(c,48+12+12+12+12,ud.savegame[4],2,10+16);
    HU_DrawMiniText(c,48+12+12+12+12+12,ud.savegame[5],2,10+16);
    HU_DrawMiniText(c,48+12+12+12+12+12+12,ud.savegame[6],2,10+16);
    HU_DrawMiniText(c,48+12+12+12+12+12+12+12,ud.savegame[7],2,10+16);
    HU_DrawMiniText(c,48+12+12+12+12+12+12+12+12,ud.savegame[8],2,10+16);
    HU_DrawMiniText(c,48+12+12+12+12+12+12+12+12+12,ud.savegame[9],2,10+16);
...
void menus(void)
{
    ...
    {
        case 25000:
            HU_DrawText(160,90,"SELECT A SAVE SPOT BEFORE",0,2+8+16);
            HU_DrawText(160,90+9,"YOU QUICK RESTORE.",0,2+8+16);
        ...
        case 20000:
            x = probe(326,190,0,0);
            HU_DrawText(160,50-8,"YOU ARE PLAYING THE SHAREWARE",0,2+8+16);
            HU_DrawText(160,59-8,"VERSION OF DUKE NUKEM 3D.  WHILE",0,2+8+16);
            HU_DrawText(160,68-8,"THIS VERSION IS REALLY COOL, YOU",0,2+8+16);
            HU_DrawText(160,77-8,"ARE MISSING OVER 75% OF THE TOTAL",0,2+8+16);
            HU_DrawText(160,86-8,"GAME, ALONG WITH OTHER GREAT EXTRAS",0,2+8+16);
            HU_DrawText(160,95-8,"AND GAMES, WHICH YOU'LL GET WHEN",0,2+8+16);
            HU_DrawText(160,104-8,"YOU ORDER THE COMPLETE VERSION AND",0,2+8+16);
            HU_DrawText(160,113-8,"GET THE FINAL TWO EPISODES.",0,2+8+16);

            HU_DrawText(160,113+8,"PLEASE READ THE 'HOW TO ORDER' ITEM",0,2+8+16);
            HU_DrawText(160,122+8,"ON THE MAIN MENU IF YOU WISH TO",0,2+8+16);
            HU_DrawText(160,131+8,"UPGRADE TO THE FULL REGISTERED",0,2+8+16);
            HU_DrawText(160,140+8,"VERSION OF DUKE NUKEM 3D.",0,2+8+16);
            HU_DrawText(160,149+16,"PRESS ANY KEY...",0,2+8+16);
        ...
        case 15000:
            ...
            HU_DrawText(160,90,"LOAD last game:",0,2+8+16);

            sprintf(text,"\"%s\"",ud.savegame[lastsavedpos]);
            HU_DrawText(160,99,text,0,2+8+16);

            HU_DrawText(160,99+9,"(Y/N)",0,2+8+16);
        ...
        case 10001:
            ...
            {
                HU_DrawText(160,43+16+16+16-12,"ENTER PASSWORD",0,2+8+16);
        ...
        case 1009:
            ...
            HU_DrawText(160,158,(char*)tempbuf,0,2+8+16);

            sprintf((char*)tempbuf,"EPISODE: %-2d / LEVEL: %-2d / SKILL: %-2d",1+volnum,1+levnum,plrskl);
            HU_DrawText(160,170,(char*)tempbuf,0,2+8+16);

            HU_DrawText(160,90,"LOAD game:",0,2+8+16);
            sprintf((char*)tempbuf,"\"%s\"",ud.savegame[current_menu-1000]);
            HU_DrawText(160,99,(char*)tempbuf,0,2+8+16);
            HU_DrawText(160,99+9,"(Y/N)",0,2+8+16);
		...
		case 1010: //loading a saved game failed
			...
			HU_DrawText(160,99,"YOU CAN'T LOAD THIS GAME",0,2+8+16);
			HU_DrawText(160,99+9,"EITHER A WONG VERSION",0,2+8+16);
			HU_DrawText(160,99+9+9,"OR BAD # OF PLAYERS OR...",0,2+8+16);
        ...
        case 1500:
            ...
            probe(186,124,0,0);
            HU_DrawText(160,90,"ABORT this game?",0,2+8+16);
            HU_DrawText(160,90+9,"(Y/N)",0,2+8+16);
        ...
        case 2009:
            ...
            HU_DrawText(160,158,text,0,2+8+16);

            sprintf(text,"EPISODE: %-2d / LEVEL: %-2d / SKILL: %-2d",1+ud.volume_number,1+ud.level_number,ud.player_skill);
            HU_DrawText(160,170,text,0,2+8+16);

            dispnames();

            HU_DrawText(160,90,"OVERWRITE previous SAVED game?",0,2+8+16);
            HU_DrawText(160,90+9,"(Y/N)",0,2+8+16);
        ...
        case 997:
			...
			switch (current_menu) {
				case 990:
					HU_DrawText(c,40,                      "ORIGINAL CONCEPT",0,2+8+16);
					HU_DrawText(c,40+9,                    "TODD REPLOGLE",0,2+8+16);
					HU_DrawText(c,40+9+9,                  "ALLEN H. BLUM III",0,2+8+16);
						
					HU_DrawText(c,40+9+9+9+9,              "PRODUCED & DIRECTED BY",0,2+8+16);
					HU_DrawText(c,40+9+9+9+9+9,            "GREG MALONE",0,2+8+16);

					HU_DrawText(c,40+9+9+9+9+9+9+9,        "EXECUTIVE PRODUCER",0,2+8+16);
					HU_DrawText(c,40+9+9+9+9+9+9+9+9,      "GEORGE BROUSSARD",0,2+8+16);

					HU_DrawText(c,40+9+9+9+9+9+9+9+9+9+9,  "BUILD ENGINE",0,2+8+16);
					HU_DrawText(c,40+9+9+9+9+9+9+9+9+9+9+9,"KEN SILVERMAN",0,2+8+16);
					break;
				case 991:
					HU_DrawText(c,40,                      "GAME PROGRAMMING",0,2+8+16);
					HU_DrawText(c,40+9,                    "TODD REPLOGLE",0,2+8+16);
						
					HU_DrawText(c,40+9+9+9,                "3D ENGINE/TOOLS/NET",0,2+8+16);
					HU_DrawText(c,40+9+9+9+9,              "KEN SILVERMAN",0,2+8+16);

					HU_DrawText(c,40+9+9+9+9+9+9,          "NETWORK LAYER/SETUP PROGRAM",0,2+8+16);
					HU_DrawText(c,40+9+9+9+9+9+9+9,        "MARK DOCHTERMANN",0,2+8+16);
					break;
				case 992:
					HU_DrawText(c,40,                      "MAP DESIGN",0,2+8+16);
					HU_DrawText(c,40+9,                    "ALLEN H BLUM III",0,2+8+16);
					HU_DrawText(c,40+9+9,                  "RICHARD GRAY",0,2+8+16);
						
					HU_DrawText(c,40+9+9+9+9,              "3D MODELING",0,2+8+16);
					HU_DrawText(c,40+9+9+9+9+9,            "CHUCK JONES",0,2+8+16);
					HU_DrawText(c,40+9+9+9+9+9+9,          "SAPPHIRE CORPORATION",0,2+8+16);

					HU_DrawText(c,40+9+9+9+9+9+9+9+9,      "ARTWORK",0,2+8+16);
					HU_DrawText(c,40+9+9+9+9+9+9+9+9+9,    "DIRK JONES, STEPHEN HORNBACK",0,2+8+16);
					HU_DrawText(c,40+9+9+9+9+9+9+9+9+9+9,  "JAMES STOREY, DAVID DEMARET",0,2+8+16);
					HU_DrawText(c,40+9+9+9+9+9+9+9+9+9+9+9,"DOUGLAS R WOOD",0,2+8+16);
					break;
				case 993:
					HU_DrawText(c,40,                      "SOUND ENGINE",0,2+8+16);
					HU_DrawText(c,40+9,                    "JIM DOSE",0,2+8+16);
						
					HU_DrawText(c,40+9+9+9,                "SOUND & MUSIC DEVELOPMENT",0,2+8+16);
					HU_DrawText(c,40+9+9+9+9,              "ROBERT PRINCE",0,2+8+16);
					HU_DrawText(c,40+9+9+9+9+9,            "LEE JACKSON",0,2+8+16);

					HU_DrawText(c,40+9+9+9+9+9+9+9,        "VOICE TALENT",0,2+8+16);
					HU_DrawText(c,40+9+9+9+9+9+9+9+9,      "LANI MINELLA - VOICE PRODUCER",0,2+8+16);
					HU_DrawText(c,40+9+9+9+9+9+9+9+9+9,    "JON ST. JOHN AS \"DUKE NUKEM\"",0,2+8+16);
					break;
				case 994:
					HU_DrawText(c,60,                      "GRAPHIC DESIGN",0,2+8+16);
					HU_DrawText(c,60+9,                    "PACKAGING, MANUAL, ADS",0,2+8+16);
					HU_DrawText(c,60+9+9,                  "ROBERT M. ATKINS",0,2+8+16);
					HU_DrawText(c,60+9+9+9,                "MICHAEL HADWIN",0,2+8+16);
					
					HU_DrawText(c,60+9+9+9+9+9,            "SPECIAL THANKS TO",0,2+8+16);
					HU_DrawText(c,60+9+9+9+9+9+9,          "STEVEN BLACKBURN, TOM HALL",0,2+8+16);
					HU_DrawText(c,60+9+9+9+9+9+9+9,        "SCOTT MILLER, JOE SIEGLER",0,2+8+16);
					HU_DrawText(c,60+9+9+9+9+9+9+9+9,      "TERRY NAGY, COLLEEN COMPTON",0,2+8+16);
					HU_DrawText(c,60+9+9+9+9+9+9+9+9+9,    "HASH INC., FORMGEN, INC.",0,2+8+16);
					break;
				case 995:
					HU_DrawText(c,49,                      "THE 3D REALMS BETA TESTERS",0,2+8+16);

					HU_DrawText(c,49+9+9,                  "NATHAN ANDERSON, WAYNE BENNER",0,2+8+16);
					HU_DrawText(c,49+9+9+9,                "GLENN BRENSINGER, ROB BROWN",0,2+8+16);
					HU_DrawText(c,49+9+9+9+9,              "ERIK HARRIS, KEN HECKBERT",0,2+8+16);
					HU_DrawText(c,49+9+9+9+9+9,            "TERRY HERRIN, GREG HIVELY",0,2+8+16);
					HU_DrawText(c,49+9+9+9+9+9+9,          "HANK LEUKART, ERIC BAKER",0,2+8+16);
					HU_DrawText(c,49+9+9+9+9+9+9+9,        "JEFF RAUSCH, KELLY ROGERS",0,2+8+16);
					HU_DrawText(c,49+9+9+9+9+9+9+9+9,      "MIKE DUNCAN, DOUG HOWELL",0,2+8+16);
					HU_DrawText(c,49+9+9+9+9+9+9+9+9+9,    "BILL BLAIR",0,2+8+16);
					break;
				case 996:
					HU_DrawText(c,32,                      "COMPANY PRODUCT SUPPORT",0,2+8+16);

					HU_DrawText(c,32+9+9,                  "THE FOLLOWING COMPANIES WERE COOL",0,2+8+16);
					HU_DrawText(c,32+9+9+9,                "ENOUGH TO GIVE US LOTS OF STUFF",0,2+8+16);
					HU_DrawText(c,32+9+9+9+9,              "DURING THE MAKING OF DUKE NUKEM 3D.",0,2+8+16);

					HU_DrawText(c,32+9+9+9+9+9+9,          "ALTEC LANSING MULTIMEDIA",0,2+8+16);
					HU_DrawText(c,32+9+9+9+9+9+9+9,        "FOR TONS OF SPEAKERS AND THE",0,2+8+16);
					HU_DrawText(c,32+9+9+9+9+9+9+9+9,      "THX-LICENSED SOUND SYSTEM",0,2+8+16);
					HU_DrawText(c,32+9+9+9+9+9+9+9+9+9,    "FOR INFO CALL 1-800-548-0620",0,2+8+16);
					
					HU_DrawText(c,32+9+9+9+9+9+9+9+9+9+9+9,"CREATIVE LABS, INC.",0,2+8+16);

					HU_DrawText(c,32+9+9+9+9+9+9+9+9+9+9+9+9+9,"THANKS FOR THE HARDWARE, GUYS.",0,2+8+16);
					break;
				case 997:
					HU_DrawText(c,50,                      "DUKE NUKEM IS A TRADEMARK OF",0,2+8+16);
					HU_DrawText(c,50+9,                    "3D REALMS ENTERTAINMENT",0,2+8+16);
						
					HU_DrawText(c,50+9+9+9,                "DUKE NUKEM",0,2+8+16);
					HU_DrawText(c,50+9+9+9+9,              "(C) 1996 3D REALMS ENTERTAINMENT",0,2+8+16);

					if (VOLUMEONE) {
					HU_DrawText(c,106,                     "PLEASE READ LICENSE.DOC FOR SHAREWARE",0,2+8+16);
					HU_DrawText(c,106+9,                   "DISTRIBUTION GRANTS AND RESTRICTIONS",0,2+8+16);
					}

					HU_DrawText(c,VOLUMEONE?134:115,       "MADE IN DALLAS, TEXAS USA",0,2+8+16);
					break;
			}
			}
...
else
{
        ...
        case 701:
            ...
            {
				case -7:
					HU_DrawText(320>>1,43+16*7+3,"*** SHORTCUT: ALT-M ***",0,2+8+16); // center-i
					break;
        ...
        case 703:
			...
			if(-2>=x && x>=(-NUMGAMEFUNCTIONS-1)) // -2 to -54 (53 values -> 0->52)
			{
				for(i=-j; i<=+j; i++)
					...
					{
						HU_DrawText(c-10,47-16+8*(i+j),gamefunctions[-x-2+i],i?0:0,2+8+16); // disp name
						if (i ||  lastkeysetup || !waiting4key || (totalclock%128 < 64)) // blink 1st key
							HU_DrawText(c+185,47-16+8*(i+j),KB_ScanCodeToString( KeyMapping[-x-2+i].key1 )?KB_ScanCodeToString( KeyMapping[-x-2+i].key1 ):"...",i?0:0,2+8+16); // center-i
						if (i || !lastkeysetup || !waiting4key || (totalclock%128 < 64)) // blink 2nd key
							HU_DrawText(c+247,47-16+8*(i+j),KB_ScanCodeToString( KeyMapping[-x-2+i].key2 )?KB_ScanCodeToString( KeyMapping[-x-2+i].key2 ):"...",i?0:0,2+8+16); // center-i
					}
			}

			if(waiting4key)
				HU_DrawText(320>>1,47-16+8*(2*j+2)-4,"*** HIT A NEW KEY ***",0,2+8+16); // center-i
			else
				HU_DrawText(320>>1,47-16+8*(2*j+2)-4,"*** HIT ENTER OR DEL KEY ***",0,2+8+16); // center-i
        ...
        case 704: // mouse button setup, frm menu 701
			...
			for(i=0; i<MAXMOUSEBUTTONS; i++)
			{
				...
				HU_DrawText(c,47+i*8,text,0,2+8+16);
				HU_DrawText(c+121,47+i*8,(MouseMapping[i]!=-1)?CONFIG_FunctionNumToName(MouseMapping[i]):"...",0,2+8+16);
			}

			HU_DrawText(320>>1,47+(MAXMOUSEBUTTONS+1)*8,"*** USE ARROWS OR DEL KEY ***",0,2+8+16);
        ...
        case 705: // digital axes setup, from menu 701
			...
			for(i=0; i<(MAXMOUSEAXES*2); i++)
			{
				*tempbuf = 0;
				...
				HU_DrawText(c,47+i*8,text,0,2+8+16);
				HU_DrawText(c+121,47+i*8,(MouseDigitalAxeMapping[i>>1][i&1]!=-1)?CONFIG_FunctionNumToName(MouseDigitalAxeMapping[i>>1][i&1]):"...",0,2+8+16);
			}

			HU_DrawText(320>>1,47+(4+1)*8,"*** USE ARROWS OR DEL KEY ***",0,2+8+16);
        ...
        case 706: // Video setup
			...
			switch(x)
			{
				case -7: // cursor idle on the FPS option (5)
					HU_DrawText(320>>1,43+16*7,"*** SHORTCUT: TYPE DNRATE ***",0,2+8+16); // center-i
					
					break;

				case -3: // cursor idle on the togglefullscreen option (1)
					HU_DrawText(320>>1,43+16*7,"*** SHORTCUT: ALT-ENTER ***",0,2+8+16); // center-i
				...
				case -2: // cursor idle on the resolution option (0)
					...
					if(lastkeysetup)
						HU_DrawText(320>>1,43+16*7,"*** HIT ENTER TO VALIDATE ***",0,2+8+16); // center-i
					else
						HU_DrawText(320>>1,43+16*7,"*** LEFT/RIGHT ARROW TO SELECT ***",0,2+8+16); // center-i
				...
				case 1:
					...
					VID_SetGameMode(validmodexdim[current_resolution],validmodeydim[current_resolution]);
					break;
            ...
            if(x==4) VID_SetBrightness(ud.brightness>>2,&ps[myconnectindex].palette[0]);
		...
		case 502: // from menu 50
            ...
            HU_DrawText(c,90,"Are you sure you want to quit?",0,2+8+16);
            HU_DrawText(c,99,"(Y/N)",0,2+8+16);
		...
		case 503:
            c = 320>>1;
            HU_DrawText(c,90,"Quit to Title?",0,2+8+16);
            HU_DrawText(c,99,"(Y/N)",0,2+8+16);
        ...
        case 601:
            ...
            HU_DrawText(160,50,(char*)tempbuf,0,2+8+16);
            HU_DrawText(160,59,"to select level",0,2+8+16);
        ...
        case 603:
            ...
            {
                if(x == fileselect)
                    HU_DrawMiniText(15 + (x/15)*54,32 + (x%15)*8,menuname[x],0,26);
                else HU_DrawMiniText(15 + (x/15)*54,32 + (x%15)*8,menuname[x],16,26);
            }
            ...
            else if(x >= 0)
            {
                ...
                P_NewGame(ud.m_volume_number,ud.m_level_number,ud.m_player_skill+1);
                P_EnterLevel(MODE_GAME);
            }
            break;
...
{                    if(boardfilename[0] == 0) break;
            ...
            if(ud.m_coop==1) HU_DrawText(c+70,57-7-9,"COOPERATIVE PLAY",0,2+8+16);
            else if(ud.m_coop==2) HU_DrawText(c+70,57-7-9,"DUKEMATCH (NO SPAWN)",0,2+8+16);
            else HU_DrawText(c+70,57-7-9,"DUKEMATCH (SPAWN)",0,2+8+16);

 if(VOLUMEONE)
            HU_DrawText(c+70,57+16-7-9,volume_names[ud.m_volume_number],0,2+8+16);
 else
            HU_DrawText(c+70,57+16-7-9,volume_names[ud.m_volume_number],0,2+8+16);
 
            HU_DrawText(c+70,57+16+16-7-9,&level_names[11*ud.m_volume_number+ud.m_level_number][0],0,2+8+16);

            if(ud.m_monsters_off == 0 || ud.m_player_skill > 0)
                HU_DrawText(c+70,57+16+16+16-7-9,skill_names[ud.m_player_skill],0,2+8+16);
            else HU_DrawText(c+70,57+16+16+16-7-9,"NONE",0,2+8+16);

            if(ud.m_coop == 0)
            {
                if(ud.m_marker)
                    HU_DrawText(c+70,57+16+16+16+16-7-9,"ON",0,2+8+16);
                else HU_DrawText(c+70,57+16+16+16+16-7-9,"OFF",0,2+8+16);
            }

            if(ud.m_coop == 1)
            {
                if(ud.m_ffire)
                    HU_DrawText(c+70,57+16+16+16+16+16-7-9,"ON",0,2+8+16);
                else HU_DrawText(c+70,57+16+16+16+16+16-7-9,"OFF",0,2+8+16);
            }
...
{            menutext(c,57+16+16+16+16+16+16-9,SHX(-7),boardfilename[0] == 0,"USER MAP");
            if( boardfilename[0] != 0 )
                HU_DrawText(c+70+44,57+16+16+16+16+16,boardfilename,0,2+8+16);
}
else
{
...
void palto(uint8_t  r,uint8_t  g,uint8_t  b,int32_t e, int present)
{
    ...
    VID_SetBrightness(ud.brightness>>2,temparray);
