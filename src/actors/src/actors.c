...
#define IFMOVING if (P_SetSprite(i, CLIPMASK0))
...
#define IFHITSECT                                                              \
    j = P_IfHitSectors(s->sectnum);                                            \
    if (j >= 0)

#define AFLAMABLE(X)                                                           \
    (X == BOX || X == TREE1 || X == TREE2 || X == TIRE || X == CONE)

short otherp;

static uint8_t actor_tog;


int32_t P_CeilingSpace(int16_t sectnum) {
...
int32_t P_FloorSpace(i16 sectnum) {
    const sectortype* s = &sector[sectnum];
    if (!(s->floorstat & 1) || s->ceilingpal) {
        return 0;
    }
    switch (s->floorpicnum) {
        case MOONSKY1:
        case BIGORBIT1:
            return 1;
        default:
...
void P_CheckAvailInven(struct player_struct* p) {
    ...
    if (s->picnum != SHRINKSPARK) {
        ...
        do {
            ...
            if (((sector[dasect].ceilingz - s->z) >> 8) < r) {
                ...
                if (d < r)
                    P_CheckHitCeiling(dasect);
                else {
                    d = klabs(wall[wall[wall[sector[dasect].wallptr].point2]
                                       .point2]
                                  .x
                              - s->x)
                        + klabs(wall[wall[wall[sector[dasect].wallptr].point2]
                                         .point2]
                                    .y
                                - s->y);
                    if (d < r)
            ...
            if (x == 0 || x >= 5 || AFLAMABLE(sj->picnum)) {
                if (s->picnum != SHRINKSPARK || (sj->cstat & 257))
                    if (P_Dist3D(s, sj) < r) {
                       ...
                       && (sj->picnum == TRIPBOMB || badguy(sj)
                           || sj->picnum == QUEBALL || sj->picnum == STRIPEBALL
                           || (sj->cstat & 257)
                           || sj->picnum == DUKELYINGDEAD)) {
                if (s->picnum == SHRINKSPARK && sj->picnum != SHARK
                    && (j == s->owner || sj->xrepeat < 24)) {
                    j = nextj;
                    continue;
                }
                ...
                if (d < r
                    ...
                    if (s->picnum != SHRINKSPARK) {
                        ...
                        if (sprite[j].picnum != TANK
                            && sprite[j].picnum != ROTATEGUN
                            && sprite[j].picnum != RECON
                            && sprite[j].picnum != BOSS1
                            && sprite[j].picnum != BOSS2
                            && sprite[j].picnum != BOSS3
                            && sprite[j].picnum != BOSS4) {
                            if (sj->xvel < 0)
                                sj->xvel = 0;
                            sj->xvel += (s->extra << 2);
                        }
                        ...
                        if (sj->picnum == PODFEM1 || sj->picnum == FEM1
                            ...
                            || sj->picnum == STATUEFLASH
                            || sj->picnum == SPACEMARINE
                            || sj->picnum == QUEBALL
                            || sj->picnum == STRIPEBALL)
                            P_CheckHitSprite(j, i);
                    } else if (s->extra == 0)
                        hittype[j].extra = 0;
...
int P_MoveSprite(
    ...
    if (sprite[spritenum].statnum == 5
        ...
        if (bg)
            B_SetSprite(spritenum, sprite[spritenum].x, sprite[spritenum].y,
                      sprite[spritenum].z);
        return 0;
    }
    ...
    h = ((tiles[sprite[spritenum].picnum].dim.height
          * sprite[spritenum].yrepeat)
         << 1);
    daz -= h;
    ...
    if (bg) {
        ...
        if (dasectnum < 0
            || (
                ...
                && (
                    (
                        hittype[spritenum].actorstayput >= 0
                        && hittype[spritenum].actorstayput != dasectnum
                    )
                    || (
                        (sprite[spritenum].picnum == BOSS2)
                    ...
                    || (
                        sector[dasectnum].lotag == 1
                        && (
                            sprite[spritenum].picnum == LIZMAN
            ...
            if (sector[dasectnum].lotag == 1
                ...
                sprite[spritenum].ang = (TRAND & 2047);
                     ...
                     && sprite[spritenum].picnum != COMMANDER)
                sprite[spritenum].ang = (TRAND & 2047);
            B_SetSprite(spritenum, oldx, oldy, sprite[spritenum].z);
            if (dasectnum < 0)
                dasectnum = 0;
    ...
    if (dasectnum >= 0)
        if ((dasectnum != sprite[spritenum].sectnum))
            B_ChangeSpriteSect(spritenum, dasectnum);
...
short P_SetSprite(short i, uint32_t cliptype) //The set sprite function
{
    spritetype* s;
    int32_t movetype;
...
void P_LotsOfMoney(spritetype* s, short n) {
    short i, j;
    for (i = n; i > 0; i--) {
        j = EGS(s->sectnum, s->x, s->y, s->z - (TRAND % (47 << 8)), MONEY, -32,
                8, 8, TRAND & 2047, 0, 0, 0, 5);
...
void P_LotsOfMail(spritetype* s, short n) {
    short i, j;
    for (i = n; i > 0; i--) {
        j = EGS(s->sectnum, s->x, s->y, s->z - (TRAND % (47 << 8)), MAIL, -32,
                8, 8, TRAND & 2047, 0, 0, 0, 5);
        sprite[j].cstat = TRAND & 12;
    }
}
...
void P_LotsOfPaper(spritetype* s, short n) {
    short i, j;
    for (i = n; i > 0; i--) {
        j = EGS(s->sectnum, s->x, s->y, s->z - (TRAND % (47 << 8)), PAPER, -32,
                8, 8, TRAND & 2047, 0, 0, 0, 5);
        sprite[j].cstat = TRAND & 12;
    }
}
...
void P_Guts(spritetype* s, short gtype, short n, short p) {
    ...
    for (j = 0; j < n; j++) {
        a = TRAND & 2047;
        i = EGS(s->sectnum, s->x + (TRAND & 255) - 128,
                s->y + (TRAND & 255) - 128, gutz - (TRAND & 8191), gtype, -32,
                sx, sy, a, 48 + (TRAND & 31), -512 - (TRAND & 2047), ps[p].i,
                5);
        if (PN == JIBS2) {
            sprite[i].xrepeat >>= 2;
            sprite[i].yrepeat >>= 2;
...
void P_SetSectInterpolate(short i) {
    ...
    for (j = startwall; j < endwall; j++) {
        P_SetInterpolation(&wall[j].x);
        P_SetInterpolation(&wall[j].y);
        k = wall[j].nextwall;
        if (k >= 0) {
            P_SetInterpolation(&wall[k].x);
            P_SetInterpolation(&wall[k].y);
            k = wall[k].point2;
            P_SetInterpolation(&wall[k].x);
            P_SetInterpolation(&wall[k].y);
        }
...
void P_ClearSectInterpolate(short i) {
    ...
    endwall = startwall + sector[SECT].wallnum;
    for (j = startwall; j < endwall; j++) {
        P_StopInterpolation(&wall[j].x);
        P_StopInterpolation(&wall[j].y);
        if (wall[j].nextwall >= 0) {
            P_StopInterpolation(&wall[wall[j].nextwall].x);
            P_StopInterpolation(&wall[wall[j].nextwall].y);
        }
    }
}
    ...
    if (hittype[sn].extra >= 0) {
        if (sprite[sn].extra >= 0) {
            ...
            if (npc->picnum == APLAYER) {
                ...
                npc->extra -= hittype[sn].extra;
                if (npc->picnum != RECON && npc->owner >= 0
...
void P_MoveDummyPlayers(void) {
    ...
    while (i >= 0) {
        ...
        if (ps[p].on_crane >= 0 || sector[ps[p].cursectnum].lotag != 1
...
void P_MovePlayers(void) //Players
{
    short i, nexti;
    int32_t otherx;
    spritetype* s;
        ...
        if (s->owner >= 0) {
            if (p->newowner >= 0) //Looking thru the camera
            {
                ...
                if (s->extra > 0) {
                    ...
                    if (ud.god == 0)
                        if (P_CeilingSpace(s->sectnum)
                            || P_FloorSpace(s->sectnum))
                            P_QuickKill(p);
                } else {
                    ...
                    if (p->wackedbyactor >= 0
                        && sprite[p->wackedbyactor].statnum < MAXSTATUS) {
                        p->ang +=
                            getincangle(
                                p->ang,
                                getangle(sprite[p->wackedbyactor].x - p->posx,
                                         sprite[p->wackedbyactor].y - p->posy))
                            >> 1;
                        p->ang &= 2047;
                    }
                }
        ...
        switch (s->picnum) {
            ...
            case MUSICANDSFX:
                ...
                if (s->lotag >= 1000 && s->lotag < 2000) {
                    x = P_Dist2D(&sprite[ps[screenpeek].i], s);
                    if (x < ht && T1 == 0) {
                    ...
                    if (x >= ht && T1 == 1) {
                        ...
                        T1 = 0;
                           ...
                           && (uint32_t) sector[s->sectnum].lotag < 9
                           && AmbienceToggle
                           && sector[SECT].floorz != sector[SECT].ceilingz) {
                    if ((soundm[s->lotag] & 2)) {
                        x = P_Dist3D(&sprite[ps[screenpeek].i], s);
                        if (x < ht && T1 == 0
                            && FX_VoiceAvailable(soundpr[s->lotag] - 1)) {
                            if (numenvsnds == NumVoices) {
                                j = headspritestat[11];
                                while (j >= 0) {
                                    if (PN == MUSICANDSFX && j != i
                                        && sprite[j].lotag < 999
                                        && hittype[j].temp_data[0] == 1
                                        && P_Dist3D(&sprite[j],
                                                    &sprite[ps[screenpeek].i])
                                               > x) {
...
void P_MoveFallers(void) {
    ...
    while (i >= 0) {
        ...
        if (T1 == 0) {
            ...
            x = s->extra;
            IFHIT {
                if (j == FIREEXT || j == RPG || j == RADIUSEXPLOSION
                    || j == SEENINE || j == OOZFILTER) {
                    if (s->extra <= 0) {
                        T1 = 1;
                        j = headspritestat[12];
                        while (j >= 0) {
    ...
    while (i >= 0) {
        ...
        IFWITHIN(CRANE, CRANE + 3) {
            ...
            if (t[0] == 0) //Waiting to check the sector
            {
                ...
                while (j >= 0) {
                    ...
                    switch (sprite[j].statnum) {
                        ...
                        case 6:
                        case 10:
                            s->ang = getangle(msx[t[4] + 1] - s->x,
                                              msy[t[4] + 1] - s->y);
                            B_SetSprite(j, msx[t[4] + 1], msy[t[4] + 1],
                                      sprite[j].z);
                            t[0]++;
                            goto BOLT;
                    }
            ...
            } else if (t[0] == 4) //Delay before going up
            {
                ...
                IFMOVING;
                if (((s->x - msx[t[4]]) * (s->x - msx[t[4]])
                     + (s->y - msy[t[4]]) * (s->y - msy[t[4]]))
                    < (128 * 128))
                    t[0]++;
            }
            ...
            B_SetSprite(msy[t[4] + 2], s->x, s->y, s->z - (34 << 8));
            ...
            if (s->owner != -1) {
                ...
                if (s->owner >= 0) {
                    B_SetSprite(s->owner, s->x, s->y, s->z);
        ...
        if (s->picnum == TRIPBOMB) {
            if (T3 > 0) {
                ...
                if (T3 == 8) {
                    ...
                    for (j = 0; j < 5; j++)
                        RANDOMSCRAP;
                    x = s->extra;
                    P_HitRadius(i, tripbombblastradius, x >> 2, x >> 1,
                                x - (x >> 2), x);
                    ...
                    j = headspritestat[5];
                    while (j >= 0) {
                        if (sprite[j].picnum == LASERLINE
                            && s->hitag == sprite[j].hitag)
            ...
            if (T1 == 32) {
                ...
                while (x > 0) {
                    j = spawn(i, LASERLINE);
                    B_SetSprite(j, sprite[j].x, sprite[j].y, sprite[j].z);
                ...
                if (j == FIREEXT || j == RPG || j == RADIUSEXPLOSION
                    ...
                    while (j >= 0) {
                        if (s->hitag == sprite[j].hitag
                            && (sprite[j].picnum == OOZFILTER
                                || sprite[j].picnum == SEENINE))
        ...
        if (s->picnum == FIREEXT) {
            ...
            for (k = 0; k < 16; k++) {
                j = EGS(SECT, SX, SY, SZ - (TRAND % (48 << 8)),
                        SCRAP3 + (TRAND & 3), -8, 48, 48, TRAND & 2047,
                        (TRAND & 63) + 64,
                        -(TRAND & 4095) - (sprite[i].zvel >> 2), i, 5);
                sprite[j].pal = 2;
            }
            ...
            if (s->hitag > 0) {
                ...
                x = s->extra;
                spawn(i, EXPLOSION2);
                P_HitRadius(i, pipebombblastradius, x >> 2, x - (x >> 1),
                            x - (x >> 2), x);
                spritesound(PIPEBOMB_EXPLODE, i);

                goto DETONATE;
            } else {
                P_HitRadius(i, seenineblastradius, 10, 15, 20, 25);
                KILLIT(i);
            }
            goto BOLT;
        }
            ...
            if (s->shade != -32 && s->shade != -33) {
                ...
                DETONATE:
                    ...
                    if ((t[3] == 1 && s->xrepeat) || s->lotag == -99) {
                        x = s->extra;
                        spawn(i, EXPLOSION2);
                        P_HitRadius(i, seenineblastradius, x >> 2, x - (x >> 1),
                                    x - (x >> 2), x);
                        spritesound(PIPEBOMB_EXPLODE, i);
                    }
        ...
        switch (s->picnum) {
            ...
            CLEAR_THE_BOLT2:
                ...
                if (l & 1)
                    s->cstat ^=
                        2; // l not defined here. Line is met in 2nd demo with l = 0.
            ...
            case TOUCHPLATE:
                ...
                if (p >= 0 && (ps[p].on_ground || s->ang == 512)) {
                    if (t[0] == 0 && !P_CheckActivatorMotion(s->lotag)) {
                        t[0] = 1;
                        t[1] = 1;
                        t[3] = !t[3];
                        P_OperateMasterSwitches(s->lotag);
                        P_OperateActivators(s->lotag, p);
                        if (s->hitag > 0) {
                            s->hitag--;
                            if (s->hitag == 0)
                                t[5] = 1;
    ...
    while (i >= 0) {
        ...
        switch (s->picnum) {
            ...
            case COOLEXPLOSION1:
                ...
                switch (s->picnum) {
                    case RPG:
                        if (hittype[i].picnum != BOSS2 && s->xrepeat >= 10
                    ...
                    if (s->picnum != FREEZEBLAST) {
                        if (s->z < hittype[i].ceilingz) {
                            ...
                            s->zvel = -1;
                                    ...
                                    && sector[s->sectnum].lotag != 1)
                                   || (s->z > hittype[i].floorz + (16 << 8)
                                       && sector[s->sectnum].lotag == 1)) {
                            j = 16384 | (s->sectnum);
                ...
                if (j != 0) {
                    if (s->picnum == COOLEXPLOSION1) {
                        if ((j & 49152) == 49152
                            && sprite[j & (MAXSPRITES - 1)].picnum != APLAYER)
                            goto BOLT;
                        s->xvel = 0;
                        s->zvel = 0;
                    }
                    ...
                    if ((j & 49152) == 49152) {
                            ...
                            if (s->picnum == SPIT) {
                                ...
                                if (ps[p].loogcnt == 0) {
                                    if (Sound[DUKE_LONGTERM_PAIN].num < 1)
                                        spritesound(DUKE_LONGTERM_PAIN,
                                                    ps[p].i);
                        ...
                        if (s->picnum != RPG && s->picnum != FREEZEBLAST
                            && s->picnum != SPIT
                            && (wall[j].overpicnum == MIRROR
                                || wall[j].picnum == MIRROR)) {
                            k = getangle(wall[wall[j].point2].x - wall[j].x,
                            ...
                            B_SetSprite(i, dax, day, daz);
                            P_CheckHitWall(i, j, s->x, s->y, s->z, s->picnum);
                    ...
                    if (s->picnum != SPIT) {
                        if (s->picnum == RPG) {
                            ...
                            P_HitRadius(i, shrinkerblastradius, 0, 0, 0, 0);
                        } else if (s->picnum != COOLEXPLOSION1
                                   && s->picnum != FREEZEBLAST
                                   && s->picnum != FIRELASER) {
                            k = spawn(i, EXPLOSION2);
                            sprite[k].xrepeat = sprite[k].yrepeat =
                                s->xrepeat >> 1;
                            if ((j & 49152) == 16384) {
                                if (s->zvel < 0) {
                                    sprite[k].cstat |= 8;
                        ...
                        if (s->picnum == RPG) {
                            ...
                            if (s->xrepeat >= 10) {
                                x = s->extra;
                                P_HitRadius(i, rpgblastradius, x >> 2, x >> 1,
                                            x - (x >> 2), x);
                            } else {
                                x = s->extra + (global_random & 3);
                                P_HitRadius(i, (rpgblastradius >> 1), x >> 2,
                                            x >> 1, x - (x >> 2), x);
                            }
                        }
                    }
                ...
                if (s->picnum == COOLEXPLOSION1) {
                    ...
                    if (s->shade >= 40)
                        KILLIT(i);
                } else if (s->picnum == RPG && sector[s->sectnum].lotag == 2
            ...
            switch (sprite[j].statnum) {
                case 10: // Player
                    ...
                    if (sprite[j].owner != -1) {
                        ...
                        if (ps[p].transporter_hold == 0
                            ...
                            if (ps[p].on_ground && sectlotag == 0 && onfloorz
                                && ps[p].jetpack_on == 0) {
                                if (sprite[i].pal == 0) {
                                ...
                                B_ChangeSpriteSect(j, sprite[OW].sectnum);
                        ...
                        if (onfloorz && sectlotag == 1 && ps[p].on_ground
                            && ps[p].posz > (sector[sect].floorz - (16 << 8))
                            && ((sync[p].bits & 2) || ps[p].poszv > 2048))
                        ...
                        if (onfloorz && sectlotag == 2
                            && ps[p].posz
                                   < (sector[sect].ceilingz + (6 << 8))) {
                            k = 1;
                        ...
                        if (k == 1) {
                            ...
                            B_ChangeSpriteSect(j, sprite[OW].sectnum);
                            B_SetSprite(ps[p].i, ps[p].posx, ps[p].posy,
                                      ps[p].posz + PHEIGHT);
                ...
                case 13:
...

                    {
                        ...
                        if (sectlotag == 0
                            && (onfloorz || klabs(sprite[j].z - SZ) < 4096)) {
                            if (sprite[OW].owner != OW && onfloorz && T1 > 0
                                && sprite[j].statnum != 5) {
                                T1++;
                                goto BOLT;
                            }
    ...
    while (i >= 0) {
        ...
        switch (s->picnum) {
            ...
            case TARGET:
                if (s->cstat & 32) {
                    ...
                    if (j >= 0) {
                        ...
                        if (k == 1) {
                            P_OperateActivators(s->lotag, -1);
                            P_OperateForceFields(i, s->lotag);
                            P_OperateMasterSwitches(s->lotag);
                        }
                    }
                }
            ...
            case RESPAWNMARKERGREEN:
                ...
                if (T1 >= (respawnitemtime >> 1)
                    && T1 < ((respawnitemtime >> 1) + (respawnitemtime >> 2)))
                ...
                if (s->xvel) {
                    ...
                    if (x < 1596) {
                        ...
                        //                        if(s->pal == 12)
                        {
                            j = getincangle(
                                ps[p].ang,
                                getangle(s->x - ps[p].posx, s->y - ps[p].posy));
                                    ...
                                    while (a >= 0) {
                                        if (sprite[a].picnum == QUEBALL
                                            ...
                                            j = getincangle(
                                                ps[p].ang,
                                                getangle(
                                                    sprite[a].x - ps[p].posx,
                                                    sprite[a].y - ps[p].posy));
                                            if (j > -64 && j < 64) {
                                                P_FindPlayer(&sprite[a], &l);
                                                if (x > l)
                                                    break;
                                            }
                                        }
            ...
            case RECON:
                ...
                if (sector[s->sectnum].ceilingstat & 1)
                    s->shade +=
                        (sector[s->sectnum].ceilingshade - s->shade) >> 1;
                else
                    s->shade += (sector[s->sectnum].floorshade - s->shade) >> 1;
                ...
                if (t[0] >= 4) {
                    ...
                    if (t[2] > (26 * 3)
                        ...
                        hittype[i].tempang +=
                            getincangle(
                                hittype[i].tempang,
                                getangle(ps[p].posx - s->x, ps[p].posy - s->y))
                            / 3;
                } else if (t[0] == 2 || t[0] == 3) {
                    t[3] = 0;
                    if (s->xvel > 0)
                    ...
                    s->ang += getincangle(s->ang, getangle(ps[p].posx - s->x,
                                                           ps[p].posy - s->y))
                              >> 2;
                }

                if (t[0] != 2 && t[0] != 3) {
                    l = P_Dist2D(&sprite[j], s);
                    if (l <= 1524) {
                        a = s->ang;
                        s->xvel >>= 1;
                    } else
                    ...
                    if (t[0] == 1
                        || t[0] == 4) // Found a locator and going with it
                    {
                        l = P_Dist3D(&sprite[j], s);
            ...
            case GREENSLIME + 7:
                ...
                if (t[0] == -5) // FROZEN
                {
                    ...
                    IFHIT {
                        if (j == FREEZEBLAST)
                            goto BOLT;
                        for (j = 16; j >= 0; j--) {
                            k = EGS(SECT, SX, SY, SZ, GLASSPIECES + (j % 3),
                                    -32, 36, 36, TRAND & 2047,
                                    32 + (TRAND & 63), 1024 - (TRAND & 1023), i,
                                    5);
                            sprite[k].pal = 1;
                        }
                        ...
                        j = getincangle(ps[p].ang, getangle(SX - ps[p].posx,
                                                            SY - ps[p].posy));
                        if (j > -128 && j < 128)
                            ps[p].quick_kick = 14;
                    }
                ...
                if (t[0] == -4) //On the player
                {
                    ...
                    if (((sync[p].bits & 4) || (ps[p].quick_kick > 0))
                        && sprite[ps[p].i].extra > 0)
                        if (ps[p].quick_kick > 0
                            || (ps[p].curr_weapon != HANDREMOTE_WEAPON
                                && ps[p].curr_weapon != HANDBOMB_WEAPON
                                && ps[p].curr_weapon != TRIPBOMB_WEAPON
                                && ps[p].ammo_amount[ps[p].curr_weapon] >= 0)) {
                            for (x = 0; x < 8; x++) {
                                j = EGS(sect, s->x, s->y, s->z - (8 << 8),
                                        SCRAP3 + (TRAND & 3), -8, 48, 48,
                                        TRAND & 2047, (TRAND & 63) + 64,
                                        -(TRAND & 4095) - (s->zvel >> 2), i, 5);
                                sprite[j].pal = 6;
                            }
                ...
                if (rnd(32)) {
                    ...
                    while (j >= 0) {
                        switch (sprite[j].picnum) {
                            ...
                            case PIGCOP:
                            case NEWBEAST:
                                if (P_Dist2D(s, &sprite[j]) < 768
                                    && (klabs(s->z - sprite[j].z)
                                        < 8192)) //Gulp them
                                {
                                    t[5] = j;
                                    t[0] = -2;
            ...
            case HEAVYHBOMB:
                ...
                if (s->picnum != BOUNCEMINE) {
                    ...
                    if (s->z
                        < hittype[i].ceilingz) // && sector[sect].lotag != 2 )
                    {
                ...
                if (t[3] == 0
                    && (s->picnum == BOUNCEMINE || s->picnum == MORTER)
                    && (j || x < 844)) {
                    t[3] = 1;
                    t[4] = 0;
                    l = 0;
            ...
            DETONATEB:
                ...
                if ((l >= 0 && ps[l].hbomb_on == 0) || t[3] == 1) {
                    ...
                    if (t[4] == 2) {
                        ...
                        P_HitRadius(i, m, x >> 2, x >> 1, x - (x >> 2), x);
                        spawn(i, EXPLOSION2);
                        if (s->zvel == 0)
                            spawn(i, EXPLOSION2BOT);
                        spritesound(PIPEBOMB_EXPLODE, i);
                        ...
                        if (ps[p].ammo_amount[HANDBOMB_WEAPON]
                            ...
                            if (ud.coop >= 1 && s->owner == i) {
                                ...
                                if (ps[p].weapreccnt < 255)
                                    ps[p].weaprecs[ps[p].weapreccnt++] =
                                        s->picnum;
                            }

                            P_AddAmmo(HANDBOMB_WEAPON, &ps[p], 1);
                            spritesound(DUKE_GET, ps[p].i);

                            if (ps[p].gotweapon[HANDBOMB_WEAPON] == 0
                                || s->owner == ps[p].i)
                                P_AddWeapon(&ps[p], HANDBOMB_WEAPON);
                ...
                if (t[1]) {
                    ...
                    switch (t[1]) {
                        case 3:
                            ...
                            P_HitRadius(i, 4096, impact_damage << 2,
...
void P_MoveExplosions(void) // STATNUM 5
{
    ...
    while (i >= 0) {
        ...
        switch (s->picnum) {
            ...
            case MAIL + 1:
            case PAPER + 1:
                hittype[i].floorz = s->z =
                    getflorzofslope(s->sectnum, s->x, s->y);
                break;
            case MONEY:
            case MAIL:
            case PAPER:
            ...
            case DUKELEG:
                ...
                if (s->z < l - (2 << 8)) {
                    if (t[1] < 2)
                        ...
                        t[1] = 0;
                        if (s->picnum == DUKELEG || s->picnum == DUKETORSO
                            || s->picnum == DUKEGUN) {
                            if (t[0] > 6)
                                t[0] = 0;
                            else
...
static void P_GutsDir(spritetype* s, short gtype, short n, short p) {
    int32_t gutz, floorz;
    short i, a, j;
    uint8_t sx, sy;
    ...
    for (j = 0; j < n; j++) {
        a = TRAND & 2047;
        i = EGS(s->sectnum, s->x, s->y, gutz, gtype, -32, sx, sy, a,
                256 + (TRAND & 127), -512 - (TRAND & 2047), ps[p].i, 5);
    }
}

void P_MoveEffectors(void) //STATNUM 3
{
        ...
        switch (st) {
                ...
                if (l && (sc->floorstat & 64)) {
                    for (p = connecthead; p >= 0; p = connectpoint2[p]) {
                        if (ps[p].cursectnum == s->sectnum
                            ...
                            rotatepoint(sprite[j].x, sprite[j].y, ps[p].posx,
                                        ps[p].posy, (q * l), &m, &x);
                    ...
                    if ((sprite[j].lotag == 14) && (sh == sprite[j].hitag)
                        ...
                        //                        if( t[4] == 1 )
                        {
                            if (hittype[j].temp_data[5] == 0)
                                hittype[j].temp_data[5] =
                                    P_Dist3D(&sprite[j], s);
                            x = sgn(P_Dist3D(&sprite[j], s)
                                    - hittype[j].temp_data[5]);
                            if (sprite[j].extra)
                                x = -x;
                            s->xvel += x;
            ...
            case 14:
                ...
                if (s->owner == -1) {
                    sprintf((char*) tempbuf,
                            "Could not find any locators for SE# 6 and 14 with "
                            "a hitag of %d.\n",
                            t[3]);
                    gameexit((char*) tempbuf);
                }

                j = P_Dist2D(&sprite[s->owner], s);
                ...
                if (s->xvel) {
                    x = getangle(sprite[s->owner].x - s->x,
                    ...
                    if (s->xvel == sc->extra) {
                        ...
                        } else if (ud.monsters_off == 0 && sc->floorpal == 0
                                   && (sc->floorstat & 1) && rnd(8)) {
                            p = P_FindPlayer(s, &x);
                            if (x < 20480) {
                                j = s->ang;
                                s->ang = getangle(s->x - ps[p].posx,
                                                  s->y - ps[p].posy);
                                P_Shoot(i, RPG);
                                s->ang = j;
                            }
                            ...
                            for (p = connecthead; p >= 0; p = connectpoint2[p])
                                if (sprite[ps[p].i].extra > 0) {
                                    ...
                                    if ((k == -1 && ud.clipping == 0)
                                        ...
                                        B_SetSprite(ps[p].i, s->x, s->y, s->z);
                                        P_QuickKill(&ps[p]);
                                    }
                                }
                    }
                    ...
                    for (p = connecthead; p >= 0; p = connectpoint2[p])
                        if (sector[ps[p].cursectnum].lotag != 2) {
                            ...
                            if (s->sectnum == sprite[ps[p].i].sectnum) {
                                rotatepoint(s->x, s->y, ps[p].posx, ps[p].posy,
                                            q, &ps[p].posx, &ps[p].posy);
                    ...
                    while (j >= 0) {
                        if (sprite[j].statnum != 10
                            ...
                            rotatepoint(s->x, s->y, sprite[j].x, sprite[j].y, q,
            ...
            case 30:
                if (s->owner == -1) {
                    ...
                    {
                        if (P_Dist2D(&sprite[s->owner], s) < (2048 - 128))
                            t[4] = 2;
                        else {
                            if (s->xvel == 0)
                                P_OperateActivators(s->hitag + (!t[3]), -1);
                            if (s->xvel < 256)
                                s->xvel += 16;
                        }
                    }
                    if (t[4] == 2) {
                        ...
                        if (s->xvel > 0)
                            ...
                            s->xvel = 0;
                            P_OperateActivators(s->hitag + (short) t[3], -1);
                            s->owner = -1;
                            s->ang += 1024;
                            t[4] = 0;
                            P_OperateForceFields(i, s->hitag);
            ...
            case 5:
                ...
                if (s->owner == -1) //Start search
                {
                    ...
                    {
                        s->owner = LocateTheLocator((short) t[4],
                                                    -1); //t[0] hold sectnum

                        if (s->owner == -1)
                            break;

                        m = P_Dist2D(&sprite[ps[p].i], &sprite[s->owner]);
                ...
                if (P_Dist2D(&sprite[s->owner], s) < 1024) {
                    short ta;
                    ta = s->ang;
                ...
                if (rnd(32)) {
                    ...
                    t[2] += getincangle(t[2] + 512, getangle(ps[p].posx - s->x,
                                                             ps[p].posy - s->y))
                            >> 2;
                    sc->ceilingshade = 0;
                }
                IFHIT {
                    ...
                    if (t[3] == 5) {
                        s->zvel += 1024;
                        HU_SetPlayerQuote(7, &ps[myconnectindex], 0);
                    }
                }
            ...
            case 9:
                ...
                if (hittype[i].temp_data[4]) {
                    ...
                    j = P_GetAnimationGoal(&sc->ceilingz);
                ...
                if (j >= 0) {
                    ...
                    while (j >= 0) {
                        if (((sprite[j].lotag) == st)
                            ...
                            else if (sector[sn].floorshade
                                     > hittype[j].temp_data[0])
                                sector[sn].floorshade = hittype[j].temp_data[0];

                            if (sector[sn].ceilingshade < m)
                                sector[sn].ceilingshade = m;
                            else if (sector[sn].ceilingshade
                                     > hittype[j].temp_data[1])
                                sector[sn].ceilingshade =
                                    hittype[j].temp_data[1];
                        }
                        j = nextspritestat[j];
                    }
                    ...
                    if (j == 1) {
                        if (t[0] > sh)
                            switch (sector[s->sectnum].lotag) {
                                ...
                                case 22:
                                case 26:
                                    if (P_GetAnimationGoal(
                                            &sector[s->sectnum].ceilingz)
                                        >= 0)
                                        break;
                                default:
                                    P_ActivateBySector(s->sectnum, i);
                                    t[0] = 0;
                                    break;
                            }
                        else
                    ...
                    if (t[3] == 1) {
                        ...
                        if (s->ang == 512) {
                            ...
                            if (ps[0].one_parallax_sectnum >= 0) {
                                sc->ceilingpicnum =
                                    sector[ps[0].one_parallax_sectnum]
                                        .ceilingpicnum;
                                sc->ceilingshade =
                                    sector[ps[0].one_parallax_sectnum]
                                        .ceilingshade;
                    ...
                    if (t[0]) //If in motion
                    {
                        ...
                        while (k >= 0) {
                            ...
                            if (sprite[k].statnum == 10
                                ...
                                B_ChangeSpriteSect(k, sprite[j].sectnum);
                                ps[p].cursectnum = sprite[j].sectnum;
            ...
            case 19: //Battlestar galactia shields
                    ...
                    if (sc->ceilingz < sc->floorz)
                        ...
                        while (j >= 0) {
                            if (sprite[j].lotag == 0 && sprite[j].hitag == sh) {
                                ...
                                sector[sprite[j].sectnum].floorpal =
                                    sector[sprite[j].sectnum].ceilingpal =
                                        sector[q].floorpal;
                                sector[sprite[j].sectnum].floorshade =
                                    sector[sprite[j].sectnum].ceilingshade =
                                        sector[q].floorshade;
                ...
                {
                    IFHITSECT {
                        HU_SetPlayerQuote(8, &ps[myconnectindex], 0);
            ...
            case 22:
                ...
                if (t[1]) {
                    if (P_GetAnimationGoal(&sector[t[0]].ceilingz) >= 0)
                        ...
                        switch (sprite[j].statnum) {
                            case 5:
                                switch (sprite[j].picnum) {
                                    ...
                                    case BLOODSPLAT3:
                                    case BLOODSPLAT4:
                                        sprite[j].xrepeat = sprite[j].yrepeat =
                                            0;
                                        j = nextj;
                                        continue;
                                    case LASERLINE:
                            ...
                            case 0:
                                ...
                                if (!(sprite[j].picnum >= CRANE
                                      && sprite[j].picnum <= (CRANE + 3))) {
                                    if (sprite[j].z
                                        > (hittype[j].floorz - (16 << 8))) {
                                        hittype[j].bposx = sprite[j].x;
                                        hittype[j].bposy = sprite[j].y;
                                        ...
                                        B_SetSprite(j, sprite[j].x, sprite[j].y,
                                                  sprite[j].z);

                                        if (sector[sprite[j].sectnum].floorstat
                                            & 2)
                                            if (sprite[j].statnum == 2)
                                                makeitfall(j);
                                    }
            ...
            case 35:
                if (sc->ceilingz > s->z)
                    for (j = 0; j < 8; j++) {
                        ...
                        P_SetSprite(k, CLIPMASK0);
                        B_SetSprite(k, sprite[k].x, sprite[k].y, sprite[k].z);
                        if (rnd(16))
                            spawn(i, EXPLOSION2);
                    }
            ...
            case 27:
                ...
                if (/*sprite[ps[p].i].extra > 0 && myconnectindex == screenpeek */
                    0) // xduke remove camera view for the 1st player (notice myconnectindex is == 1 when replaying, dosent matter who recorded)
                {
                    if (t[0] < 0) {
                        ud.camerasprite = i;
                        t[0]++;
            ...
            case 28:
                ...
                if (T1 == 0) {
                    ...
                    if (T3 > T2) {
                        ...
                        j = headspritestat[0];
                        while (j >= 0) {
                            if (sprite[j].picnum == NATURALLIGHTNING
                                && sprite[j].hitag == s->hitag)
                                sprite[j].cstat |= 32768;
                            j = nextspritestat[j];
                        }
                        ...
                        while (j >= 0) {
                            if (sprite[j].picnum == NATURALLIGHTNING
                                ...
                                if (rnd(32) && (T3 & 1)) {
                                    ...
                                    p = P_FindPlayer(s, &x);
                                    x = P_Dist2D(&sprite[ps[p].i], &sprite[j]);
                                    if (x < 768) {
                                        if (Sound[DUKE_LONGTERM_PAIN].num < 1)
                                            spritesound(DUKE_LONGTERM_PAIN,
                                                        ps[p].i);
    ...
    for (i = headspritestat[3]; i >= 0; i = nextspritestat[i]) {
        ...
        alignflorslope(s->sectnum, wal->x, wal->y,
