...
uint8_t waterpal[768], slimepal[768], titlepal[768], drealms[768],
    endingpal[768];
...
static char* grpVersion2char_from_crc(unsigned int crc32_grp_to_identify) {
    char* id = crc32lookup[MAX_KNOWN_GRP].name; // unknown version
    for (int i = 0; i < MAX_KNOWN_GRP; i++) {
...
void getpackets(void) {
    ...
    sampletimer();
    if (qe == 0 && KB_KeyPressed(sc_LeftControl) && KB_KeyPressed(sc_LeftAlt)
        && KB_KeyPressed(sc_Delete)) {
        qe = 1;
        gameexit("Quick Exit.");
    }
        ...
        switch (packbuf[0]) {
            ...
            case 0: //[0] (receive master sync buffer)
                ...
                if ((movefifoend[other] & (TIMERUPDATESIZ - 1)) == 0)
                    for (i = connectpoint2[connecthead]; i >= 0;
                         i = connectpoint2[i]) {
                ...
                osyn = (input*) &inputfifo[(movefifoend[connecthead] - 1)
                                           & (MOVEFIFOSIZ - 1)][0];
                nsyn = (input*) &inputfifo[(movefifoend[connecthead])
                                           & (MOVEFIFOSIZ - 1)][0];
                ...
                for (i = connecthead; i >= 0; i = connectpoint2[i]) {
                    ...
                    if (l & 2)
                        ...
                        j += 2;
                    if (l & 4)
                        nsyn[i].avel = (int8_t) packbuf[j++];
                    if (l & 8)
                        nsyn[i].bits = ((nsyn[i].bits & 0xffffff00)
                                        | ((int32_t) packbuf[j++]));
                    if (l & 16)
                        nsyn[i].bits = ((nsyn[i].bits & 0xffff00ff)
                                        | ((int32_t) packbuf[j++]) << 8);
                    if (l & 32)
                        nsyn[i].bits = ((nsyn[i].bits & 0xff00ffff)
                                        | ((int32_t) packbuf[j++]) << 16);
                    if (l & 64)
                        nsyn[i].bits = ((nsyn[i].bits & 0x00ffffff)
                                        | ((int32_t) packbuf[j++]) << 24);
                    if (l & 128)
                        nsyn[i].horz = (int8_t) packbuf[j++];
                ...
                while (j != packbufleng) {
                    for (i = connecthead; i >= 0; i = connectpoint2[i])
                        if (i != myconnectindex) {
                            syncval[i][syncvalhead[i] & (MOVEFIFOSIZ - 1)] =
                                packbuf[j];
                            syncvalhead[i]++;
                        }
                    j++;
            ...
            case 1: //[1] (receive slave sync buffer)
                ...
                osyn = (input*) &inputfifo[(movefifoend[other] - 1)
                                           & (MOVEFIFOSIZ - 1)][0];
                nsyn = (input*) &inputfifo[(movefifoend[other])
                                           & (MOVEFIFOSIZ - 1)][0];
                ...
                if (k & 2)
                    ...
                    j += 2;
                if (k & 4)
                    nsyn[other].avel = (int8_t) packbuf[j++];
                if (k & 8)
                    nsyn[other].bits = ((nsyn[other].bits & 0xffffff00)
                                        | ((int32_t) packbuf[j++]));
                if (k & 16)
                    nsyn[other].bits = ((nsyn[other].bits & 0xffff00ff)
                                        | ((int32_t) packbuf[j++]) << 8);
                if (k & 32)
                    nsyn[other].bits = ((nsyn[other].bits & 0xff00ffff)
                                        | ((int32_t) packbuf[j++]) << 16);
                if (k & 64)
                    nsyn[other].bits = ((nsyn[other].bits & 0x00ffffff)
                                        | ((int32_t) packbuf[j++]) << 24);
                if (k & 128)
                    nsyn[other].horz = (int8_t) packbuf[j++];
                movefifoend[other]++;

                while (j != packbufleng) {
                    syncval[other][syncvalhead[other] & (MOVEFIFOSIZ - 1)] =
                        packbuf[j++];
                    syncvalhead[other]++;
                }

                for (i = 1; i < movesperpacket; i++) {
                    copybufbyte(&nsyn[other],
                                &inputfifo[movefifoend[other]
                                           & (MOVEFIFOSIZ - 1)][other],
                                sizeof(input));
                    movefifoend[other]++;
                }
            ...
            case 5:
                ...
                P_NewGame(ud.volume_number, ud.level_number, ud.player_skill);
                ud.coop = ud.m_coop;

                P_EnterLevel(MODE_GAME);
            ...
            case 6:                                     // get names
                ...
                if (packbuf[1] == BYTEVERSION_27
                    || packbuf[1] == BYTEVERSION_117) {
                    // Old rancid was using either BYTEVERSION_27 or BYTEVERSION_117
                    Error(EXIT_SUCCESS,
                          "STOP: Your opponent is using an obsolete version\n"
                          "Please ask him to update to xDuke v%d.%d!\n",
                          CLEAN_DUKE_REV_X, CLEAN_DUKE_REV_DOT_Y);
                }
                break;
            ...
            case 16:
                movefifoend[other] = movefifoplc = movefifosendplc =
                    fakemovefifoplc = 0;
            ...
            case 17:
                ...
                if (j > packbufleng) {
                    printf("INVALID GAME PACKET!!! (%d too many bytes) (j= %d, "
                           "packbuflen= %d, type: %d)\n",
            ...
            case 132: // get map CRC of opponents (to debug out of synch)
                ud.mapCRC[other] =
                    (uint16_t) packbuf[1] + (uint16_t) (packbuf[2] << 8);
                break;

            case 133: // client refused to disable the autoaim by host

                Error(EXIT_SUCCESS, "One or more players refused to play with "
                                    "AutoAim OFF because this breaks\n"
                                    "the official Duke's gameplay. Please "
                                    "restart without this option...\n");

                break;

            case 134: // Get GRP CRC32 + Con size + exeCRC + conCRC
                memcpy(ud.groupefil_crc32[other], packbuf + 1,
                       sizeof(groupefil_crc32));
                memcpy(ud.conSize + other,
                       packbuf + 1 + sizeof(groupefil_crc32),
                       sizeof(ud.conSize[0]));
                memcpy(ud.conCRC + other,
                       packbuf + 1 + sizeof(groupefil_crc32)
                           + sizeof(ud.conSize[0]),
                       sizeof(ud.conCRC[0]));
                memcpy(ud.exeCRC + other,
                       packbuf + 1 + sizeof(groupefil_crc32)
                           + sizeof(ud.conSize[0]) + sizeof(ud.conCRC[0]),
                       sizeof(ud.exeCRC[0]));
                break;
...
void faketimerhandler() {
    ...
    if (movefifoend[myconnectindex] & (movesperpacket - 1)) {
        copybufbyte(&inputfifo[(movefifoend[myconnectindex] - 1)
                               & (MOVEFIFOSIZ - 1)][myconnectindex],
                    &inputfifo[movefifoend[myconnectindex] & (MOVEFIFOSIZ - 1)]
                              [myconnectindex],
                    sizeof(input));
            ...
            for (i = connecthead; i >= 0; i = connectpoint2[i])
                if (i != myconnectindex) {
                    ...
                    if (ud.playerai)
                        P_ComputerGetInput(
                            i,
                            &inputfifo[movefifoend[i] & (MOVEFIFOSIZ - 1)][i]);
                    movefifoend[i]++;
                }
        return;
    }
    ...
    if (networkmode == 1) {
        ...
        osyn = (input*) &inputfifo[(movefifoend[myconnectindex] - 2)
                                   & (MOVEFIFOSIZ - 1)][myconnectindex];
        nsyn = (input*) &inputfifo[(movefifoend[myconnectindex] - 1)
                                   & (MOVEFIFOSIZ - 1)][myconnectindex];
    ...
    while (1) //Master
    {
        ...
        osyn =
            (input*) &inputfifo[(movefifosendplc - 1) & (MOVEFIFOSIZ - 1)][0];
...
void dispVersion(void) {
    ...
    rotatesprite(
        0 << 16, 0 << 16, 65536l << 5, 0, BLANK, 8, 0, 1 + 2 + 8 + 16 + 64,
        scale(0, xdim, 320), scale(26, ydim, 200),
        scale(320 - 0, xdim, 320) - 1,
        scale(
            200 - ((ud.multimode > 4) ? (161 - 1 * 7) - stepy : (161 - 1 * 7)),
            ydim, 200)
            - 1);
    ...
    for (i = connecthead; i >= 0; i = connectpoint2[i]) {
        ...
        HU_DrawMiniText(offx + (stepx * (i & 3)),
                        offy + 0 + ((i & 4) >> 2) * stepy, text,
                        sprite[ps[i].i].pal, 2 + 8 + 16);
        ...
        HU_DrawMiniText(offx + (stepx * (i & 3)),
                        offy + 7 + ((i & 4) >> 2) * stepy, text, COLOR_ON,
                        2 + 8 + 16);
    }
}
...
void checksync(void) {
    ...
    if (syncstat) {
        HU_DrawMiniText(21, 30 + 35 + 30, "Out Of Sync - Please restart game",
                        COLOR_ON, 2 + 8 + 16);
        ...
        for (i = connecthead; i >= 0; i = connectpoint2[i]) {
            if (ud.mapCRC[connecthead] != ud.mapCRC[i]) {
                HU_DrawMiniText(
                    21, 30 + 42 + 30,
                    "Map CRC mismatching. Please use exactly the same map.",
                    COLOR_ON, 2 + 8 + 16);
                dispVersion();
            } else
                HU_DrawMiniText(21, 30 + 42 + 30,
                                "Verify the con files. Close your P2P if any",
                                COLOR_ON, 2 + 8 + 16);
        }
    }
    ...
    if (syncstate) {
        ...
        HU_DrawMiniText(21, 30 + 35 + 30, "Missed Network packet!", COLOR_ON,
                        2 + 8 + 16);
    }
}
...
void myos(int32_t x, int32_t y, short tilenum, int8_t shade,
          uint8_t orientation) {
    uint8_t p;
    short a;
    ...
    rotatesprite(x << 16, y << 16, 65536L, a, tilenum, shade, p,
                 2 | orientation, windowx1, windowy1, windowx2, windowy2);
}

void myospal(int32_t x, int32_t y, short tilenum, int8_t shade,
...
void invennum(int32_t x, int32_t y, uint8_t num1, uint8_t ha, uint8_t sbits) {
    ...
    if (num1 > 99) {
        rotatesprite((x - 4) << 16, y << 16, 65536L, 0,
                     THREEBYFIVE + dabuf[0] - '0', ha, 0, sbits, 0, 0, xdim - 1,
                     ydim - 1);
        rotatesprite((x) << 16, y << 16, 65536L, 0,
                     THREEBYFIVE + dabuf[1] - '0', ha, 0, sbits, 0, 0, xdim - 1,
                     ydim - 1);
        rotatesprite((x + 4) << 16, y << 16, 65536L, 0,
                     THREEBYFIVE + dabuf[2] - '0', ha, 0, sbits, 0, 0, xdim - 1,
                     ydim - 1);
    } else if (num1 > 9) {
        rotatesprite((x) << 16, y << 16, 65536L, 0,
                     THREEBYFIVE + dabuf[0] - '0', ha, 0, sbits, 0, 0, xdim - 1,
...
void weaponnum(short ind, int32_t x, int32_t y, int32_t num1, int32_t num2,
               uint8_t ha) {
    char dabuf[80] = {0};
    ...
    rotatesprite((x + 9) << 16, y << 16, 65536L, 0, THREEBYFIVE + 11, ha, 0,
                 10 + 128, 0, 0, xdim - 1, ydim - 1);
    ...
    if (num1 > 9) {
        rotatesprite((x) << 16, y << 16, 65536L, 0,
                     THREEBYFIVE + dabuf[0] - '0', ha, 0, 10 + 128, 0, 0,
                     xdim - 1, ydim - 1);
        rotatesprite((x + 4) << 16, y << 16, 65536L, 0,
                     THREEBYFIVE + dabuf[1] - '0', ha, 0, 10 + 128, 0, 0,
                     xdim - 1, ydim - 1);
    } else
        rotatesprite((x + 4) << 16, y << 16, 65536L, 0,
                     THREEBYFIVE + dabuf[0] - '0', ha, 0, 10 + 128, 0, 0,
    ...
    if (num2 > 9) {
        rotatesprite((x + 13) << 16, y << 16, 65536L, 0,
                     THREEBYFIVE + dabuf[0] - '0', ha, 0, 10 + 128, 0, 0,
                     xdim - 1, ydim - 1);
        rotatesprite((x + 17) << 16, y << 16, 65536L, 0,
                     THREEBYFIVE + dabuf[1] - '0', ha, 0, 10 + 128, 0, 0,
                     xdim - 1, ydim - 1);
    } else
        rotatesprite((x + 13) << 16, y << 16, 65536L, 0,
...
void weaponnum999(uint8_t ind, int32_t x, int32_t y, int32_t num1, int32_t num2,
                  uint8_t ha) {
    char dabuf[80] = {0};

    rotatesprite((x - 7) << 16, y << 16, 65536L, 0, THREEBYFIVE + ind + 1,
                 ha - 10, 7, 10 + 128, 0, 0, xdim - 1, ydim - 1);
    rotatesprite((x - 4) << 16, y << 16, 65536L, 0, THREEBYFIVE + 10, ha, 0,
                 10 + 128, 0, 0, xdim - 1, ydim - 1);
    rotatesprite((x + 13) << 16, y << 16, 65536L, 0, THREEBYFIVE + 11, ha, 0,
                 10 + 128, 0, 0, xdim - 1, ydim - 1);
    ...
    if (num1 > 99) {
        ...
        rotatesprite((x + 4) << 16, y << 16, 65536L, 0,
                     THREEBYFIVE + dabuf[1] - '0', ha, 0, 10 + 128, 0, 0,
                     xdim - 1, ydim - 1);
        rotatesprite((x + 8) << 16, y << 16, 65536L, 0,
                     THREEBYFIVE + dabuf[2] - '0', ha, 0, 10 + 128, 0, 0,
                     xdim - 1, ydim - 1);
    } else if (num1 > 9) {
        rotatesprite((x + 4) << 16, y << 16, 65536L, 0,
                     THREEBYFIVE + dabuf[0] - '0', ha, 0, 10 + 128, 0, 0,
    ...
    if (num2 > 99) {
        rotatesprite((x + 17) << 16, y << 16, 65536L, 0,
                     THREEBYFIVE + dabuf[0] - '0', ha, 0, 10 + 128, 0, 0,
                     xdim - 1, ydim - 1);
        rotatesprite((x + 21) << 16, y << 16, 65536L, 0,
                     THREEBYFIVE + dabuf[1] - '0', ha, 0, 10 + 128, 0, 0,
                     xdim - 1, ydim - 1);
        rotatesprite((x + 25) << 16, y << 16, 65536L, 0,
                     THREEBYFIVE + dabuf[2] - '0', ha, 0, 10 + 128, 0, 0,
                     xdim - 1, ydim - 1);
    } else if (num2 > 9) {
        rotatesprite((x + 17) << 16, y << 16, 65536L, 0,
...
void displayfragbar(void) {
    ...
    rotatesprite(0, 0, 65600L, 0, FRAGBAR, 0, 0, 2 + 8 + 16 + 64 + 128, 0, 0,
                 xdim - 1, ydim - 1);
    if (j >= 4)
        rotatesprite(319, (8) << 16, 65600L, 0, FRAGBAR, 0, 0,
                     10 + 16 + 64 + 128, 0, 0, xdim - 1, ydim - 1);
    if (j >= 8)
        rotatesprite(319, (16) << 16, 65600L, 0, FRAGBAR, 0, 0,
                     10 + 16 + 64 + 128, 0, 0, xdim - 1, ydim - 1);
    if (j >= 12)
        rotatesprite(319, (24) << 16, 65600L, 0, FRAGBAR, 0, 0,
                     10 + 16 + 64 + 128, 0, 0, xdim - 1, ydim - 1);

    for (i = connecthead; i >= 0; i = connectpoint2[i]) {
        HU_DrawMiniText(21 + (73 * (i & 3)), 2 + ((i & 28) << 1),
                        &ud.user_name[i][0], sprite[ps[i].i].pal,
                        2 + 8 + 16 + 128);
        sprintf(text, "%d", ps[i].frag - ps[i].fraggedself);
        HU_DrawMiniText(17 + 50 + (73 * (i & 3)), 2 + ((i & 28) << 1), text,
                        sprite[ps[i].i].pal, 2 + 8 + 16 + 128);
    }
}
...
void tics(short offx, short offy, short color) {
    static int32_t frameval[AVERAGEFRAMES];
    static int32_t framecnt = 0;
    static int32_t fpsAvg = 0;
    static int32_t savedFps = 0;
    ...
    int32_t i = totalclock;

    if (i != frameval[framecnt]) {
        int32_t currentFps =
            (TICRATE * AVERAGEFRAMES) / (i - frameval[framecnt]);
    ...
    HU_DrawMiniText(offx, offy, text, color, 2 + 8 + 16 + 128);
}
...
void showtwoscreens(void) {
    ...
    if (VOLUMEONE) {
        ...
        rotatesprite(0, 0, 65536L, 0, 3291, 0, 0, 2 + 8 + 16 + 64, 0, 0,
                     xdim - 1, ydim - 1);
        ...
        while (!KB_KeyWaiting())
            ; // getpackets(); // Net already off. Trying to get packets here makes sporadic crash..
        ...
        rotatesprite(0, 0, 65536L, 0, 3290, 0, 0, 2 + 8 + 16 + 64, 0, 0,
                     xdim - 1, ydim - 1);
        nextpage();
        ...
        R_ClearView(0L);
        rotatesprite(0, 0, 65536L, 0, TENSCREEN, 0, 0, 2 + 8 + 16 + 64, 0, 0,
                     xdim - 1, ydim - 1);
        nextpage();
...
void gameexit(char* msg) {
    ...
    if (ud.m_recstat != 2 && ud.last_level >= 0 && playerswhenstarted > 1
        && ud.coop != 1 && *t == ' ') {
        dobonus(1);
        // CTW - MODIFICATION
        //      setgamemode();
...
void typemode(void) {
    ...
    if (ps[myconnectindex].gm & MODE_SENDTOWHOM) {
        if (sendmessagecommand != -1 || ud.multimode < 3
            ...
            j = 50;
            HU_DrawText(320 >> 1, j, "SEND MESSAGE TO...", 0, 2 + 8 + 16);
            j += 8;
            for (i = connecthead; i >= 0; i = connectpoint2[i])
            //                for(i=0;i<ud.multimode;i++)
            {
                if (i == myconnectindex) {
                    HU_MiniTextShade((320 >> 1) - 40 + 1, j + 1,
                                     "A/ENTER - ALL", 26, 0, 2 + 8 + 16);
                    HU_DrawMiniText((320 >> 1) - 40, j, "A/ENTER - ALL", 0,
                                    2 + 8 + 16);
                    j += 7;
                } else {
                    sprintf(buf, "      %d - %s", i + 1, ud.user_name[i]);
                    HU_MiniTextShade((320 >> 1) - 40 - 6 + 1, j + 1, buf, 26, 0,
                                     2 + 8 + 16);
                    HU_DrawMiniText((320 >> 1) - 40 - 6, j, buf, 0, 2 + 8 + 16);
                    j += 7;
                }
            }
            HU_MiniTextShade((320 >> 1) - 40 - 4 + 1, j + 1, "    ESC - Abort",
                             26, 0, 2 + 8 + 16);
            HU_DrawMiniText((320 >> 1) - 40 - 4, j, "    ESC - Abort", 0,
                            2 + 8 + 16);
            j += 7;
            ...
            if (ud.screen_size > 0)
                ...
                j = 200 - 8;
            HU_DrawText(320 >> 1, j, typebuf, 0, 2 + 8 + 16);
...
void view(struct player_struct* pp, int32_t* vx, int32_t* vy, int32_t* vz,
          short* vsectnum, short ang, short horiz) {
    spritetype* sp;
...
void drawbackground(void) {
    ...
    for (y = y1; y < y2; y += 128)
        for (x = 0; x < xdim; x += 128)
            rotatesprite(x << 16, y << 16, 65536L, 0, dapicnum, 8, 0,
                         8 + 16 + 64 + 128, 0, y1, xdim - 1, y2 - 1);

    // FIX_00081: Screen border in menu
    if (ud.screen_size > 8
        && (ps[myconnectindex].gm & MODE_GAME
            || ud.recstat == 2)) // ud.recstat == 2 => playing demo
    {
        y = 0;
        if (ud.coop != 1) {
            if (ud.multimode > 1)
        ...
        for (y = y1 + 4; y < y2 - 4; y += 64) {
            rotatesprite(x1 << 16, y << 16, 65536L, 0, VIEWBORDER, 0, 0,
                         8 + 16 + 64 + 128, x1, y1, x2, y2);
            rotatesprite((x2 + 1) << 16, (y + 64) << 16, 65536L, 1024,
                         VIEWBORDER, 0, 0, 8 + 16 + 64 + 128, x1, y1, x2, y2);
        }

        for (x = x1 + 4; x < x2 - 4; x += 64) {
            rotatesprite((x + 64) << 16, y1 << 16, 65536L, 512, VIEWBORDER, 0,
                         0, 8 + 16 + 64 + 128, x1, y1, x2, y2);
            rotatesprite(x << 16, (y2 + 1) << 16, 65536L, 1536, VIEWBORDER, 0,
                         0, 8 + 16 + 64 + 128, x1, y1, x2, y2);
        }

        rotatesprite(x1 << 16, y1 << 16, 65536L, 0, VIEWBORDER + 1, 0, 0,
                     8 + 16 + 64 + 128, x1, y1, x2, y2);
        rotatesprite((x2 + 1) << 16, y1 << 16, 65536L, 512, VIEWBORDER + 1, 0,
                     0, 8 + 16 + 64 + 128, x1, y1, x2, y2);
        rotatesprite((x2 + 1) << 16, (y2 + 1) << 16, 65536L, 1024,
                     VIEWBORDER + 1, 0, 0, 8 + 16 + 64 + 128, x1, y1, x2, y2);
        rotatesprite(x1 << 16, (y2 + 1) << 16, 65536L, 1536, VIEWBORDER + 1, 0,
                     0, 8 + 16 + 64 + 128, x1, y1, x2, y2);
    }
}
...
static void SE40_Draw(int spnum, int32_t x, int32_t y, int32_t z, short a,
                      short h, int32_t smoothratio) {
    int i = 0, j = 0, k = 0;
    ...
    */
    ...
    for (j = 0; j < MAXSPRITES; j++) {
        if (sprite[j].picnum == 1 && sprite[j].lotag == k
            && sprite[j].hitag == sprite[floor1].hitag) {
            floor2 = j;
            ok++;
            break;
    ...
    {
        if (sprite[j].picnum == 1 && sprite[j].lotag == k + 2
            && sprite[j].hitag == sprite[floor1].hitag) {
            if (k == 40) {
                tempsectorz[sprite[j].sectnum] =
                    sector[sprite[j].sectnum].floorz;
                sector[sprite[j].sectnum].floorz +=
                    (((z - sector[sprite[j].sectnum].floorz) / 32768) + 1)
                    * 32768;
                tempsectorpicnum[sprite[j].sectnum] =
                    sector[sprite[j].sectnum].floorpicnum;
            ...
            if (k == 41) {
                ...
                sector[sprite[j].sectnum].ceilingz +=
                    (((z - sector[sprite[j].sectnum].ceilingz) / 32768) - 1)
                    * 32768;
                tempsectorpicnum[sprite[j].sectnum] =
                    sector[sprite[j].sectnum].ceilingpicnum;
    ...
    offy = y - sprite[i].y;
    i = floor2;
    drawrooms(offx + sprite[i].x, offy + sprite[i].y, z, a, h,
              sprite[i].sectnum);
...
void displayrooms(short snum, int32_t smoothratio) {
    ...
    P_DoInterpolations(smoothratio);
    ...
    if (ud.camerasprite >= 0) {
        ...
        cang =
            hittype[ud.camerasprite].tempang
            + mulscale16(
                (int32_t) (((s->ang + 1024 - hittype[ud.camerasprite].tempang)
                            & 2047)
                           - 1024),
                smoothratio);
        ...
        if (screencapt) {
            ...
            if (tiles[MAXTILES - 1].data == NULL)
                allocache(&tiles[MAXTILES - 1].data, 100 * 160,
                          &tiles[MAXTILES - 1].lock);
            ...
            if (tiles[MAXTILES - 2].data == NULL)
                allocache(&tiles[MAXTILES - 2].data, 320L * 320L,
                          &tiles[MAXTILES - 2].lock);
            if ((tang & 1023) == 0)
                setviewtotile(MAXTILES - 2, 200L >> (1 - ud.detail),
                              320L >> (1 - ud.detail));
            else
                setviewtotile(MAXTILES - 2, 320L >> (1 - ud.detail),
                              320L >> (1 - ud.detail));
            if ((tang & 1023)
                == 512) { //Block off unscreen section of 90ø tilted screen
        ...
        if ((snum == myconnectindex) && (numplayers > 1)) {
            ...
            cang = omyang
                   + mulscale16(
                       (int32_t) (((myang + 1024 - omyang) & 2047) - 1024),
                       smoothratio);
            choriz = omyhoriz + omyhorizoff
                     + mulscale16((int32_t) (myhoriz + myhorizoff - omyhoriz
                                             - omyhorizoff),
                                  smoothratio);
            sect = mycursectnum;
        } else {
            cposx = p->oposx
                    + mulscale16((int32_t) (p->posx - p->oposx), smoothratio);
            cposy = p->oposy
                    + mulscale16((int32_t) (p->posy - p->oposy), smoothratio);
            cposz = p->oposz
                    + mulscale16((int32_t) (p->posz - p->oposz), smoothratio);
            cang = p->oang
                   + mulscale16(
                       (int32_t) (((p->ang + 1024 - p->oang) & 2047) - 1024),
                       smoothratio);
            choriz = p->ohoriz + p->ohorizoff
                     + mulscale16((int32_t) (p->horiz + p->horizoff - p->ohoriz
                                             - p->ohorizoff),
                                  smoothratio);
        }
        cang += p->look_ang;
        ...
        if (p->newowner >= 0) {
            ...
            cposz +=
                p->opyoff
                + mulscale16((int32_t) (p->pyoff - p->opyoff), smoothratio);
        else
        ...
        if ((gotpic[MIRROR >> 3] & (1 << (MIRROR & 7))) > 0) {
            ...
            if (wall[mirrorwall[i]].overpicnum == MIRROR) {
                preparemirror(cposx, cposy, cang, mirrorwall[i], &tposx, &tposy,
                              &tang);

                j = visibility;
                visibility = (j >> 1) + (j >> 2);

                drawrooms(tposx, tposy, cposz, tang, choriz,
                          mirrorsector[i] + MAXSECTORS);
        ...
        if (screencapt == 1) {
            ...
            if (ud.screen_tilting)
                ...
                tang = 0;
            setviewback();
            tiles[MAXTILES - 2].anim_flags &= 0xff0000ff;
            i = (tang & 511);
            if (i > 256)
                i = 512 - i;
            i = sintable[i + 512] * 8 + sintable[i] * 5L;
            ...
            rotatesprite(160 << 16, 100 << 16, i, tang + 512, MAXTILES - 2, 0,
                         0, 4 + 2 + 64, windowx1, windowy1, windowx2, windowy2);
...
short EGS(
    ...
    i = B_InsertSprite(whatsect, s_ss);

    if (i < 0) {
        gameexit(
            " Too many sprites spawned. This may happen (for any duke port) if "
            "you have hacked the steroids trail in the *.con files. If so, "
            "delete your *.con files to use the internal ones and try again.");
    }
...
short spawn(short j, short pn) {
    ...
    if (j >= 0) {
        i = EGS(sprite[j].sectnum, sprite[j].x, sprite[j].y, sprite[j].z, pn, 0,
                0, 0, 0, 0, 0, j, 0);
        ...
        if (PN != SPEAKER && PN != LETTER && PN != DUCK && PN != TARGET
            && PN != TRIPBOMB && PN != VIEWSCREEN && PN != VIEWSCREEN2
            && (CS & 48))
            if (!(PN >= CRACK1 && PN <= CRACK4)) {
                ...
                if (wallswitchcheck(i) == 1 && (CS & 16)) {
                    ...
                    CS |= 257;
                    if (sprite[i].pal && PN != ACCESSSWITCH
                        && PN != ACCESSSWITCH2)
                        sprite[i].pal = 0;
                    return i;
                }
    ...
    switch (sp->picnum) {
            ...
            if (actorscrptr[sp->picnum]) {
                ...
                if (actortype[sp->picnum] & 3) {
                    ...
                    if (j >= 0) {
                        ...
                        B_ChangeSpriteStat(i, 1);
                    } else
                        B_ChangeSpriteStat(i, 2);
                } else {
                    sp->clipdist = 40;
                    sp->owner = i;
                    B_ChangeSpriteStat(i, 1);
        ...
        case WATERSPLASH2:
            if (j >= 0) {
                B_SetSprite(i, sprite[j].x, sprite[j].y, sprite[j].z);
        ...
        case LASERLINE:
            ...
            if (j >= 0)
                sp->ang = hittype[j].temp_data[5] + 512;
            B_ChangeSpriteStat(i, 5);
            break;
        ...
        case PUKE: {
            ...
            if (j >= 0 && sp->picnum != PUKE) {
                if (sprite[j].pal == 1)
                    sp->pal = 1;
                else if (sprite[j].pal != 6 && sprite[j].picnum != NUKEBARREL
                         && sprite[j].picnum != TIRE) {
        ...
        case MAIL:
        case PAPER:
            if (sp->picnum == MONEY || sp->picnum == MAIL
                || sp->picnum == PAPER) {
            ...
            if (j >= 0) {
                ...
                if (sprite[j].picnum == APLAYER) {
                    ...
                    if (sp->picnum == SHOTGUNSHELL)
                        sp->z =
                            (6 << 8) + ps[snum].pyoff + ps[snum].posz
                            - ((ps[snum].horizoff + ps[snum].horiz - 100) << 4);
                    else
                        sp->z =
                            (3 << 8) + ps[snum].pyoff + ps[snum].posz
                            - ((ps[snum].horizoff + ps[snum].horiz - 100) << 4);
                    sp->zvel = -(TRAND & 255);
                } else {
                    a = sp->ang;
        ...
        case CRANE:
            ...
            while (s >= 0) {
                if (sprite[s].picnum == CRANEPOLE && SHT == (sprite[s].hitag)) {
                    ...
                    B_SetSprite(s, sprite[s].x, sprite[s].y, sprite[s].z);
                    break;
                }
                s = nextspritestat[s];
            }
        ...
        case WATERDRIP:
            if ((j >= 0 && sprite[j].statnum == 10) || sprite[j].statnum == 1) {
                ...
                if (sprite[j].pal != 1) {
                    ...
                    sp->z -= (13 << 8);
                sp->ang = getangle(ps[connecthead].posx - sp->x,
                                   ps[connecthead].posy - sp->y);
                sp->xvel = 48 - (TRAND & 31);
                P_SetSprite(i, CLIPMASK0);
            } else if (j == -1) {
                sp->z += (4 << 8);
        ...
        case SHARK:
            ...
            if (sp->picnum == BOSS4STAYPUT || sp->picnum == BOSS1
                || sp->picnum == BOSS2 || sp->picnum == BOSS1STAYPUT
                || sp->picnum == BOSS3 || sp->picnum == BOSS4) {
        ...
        case SECTOREFFECTOR:
            ...
            switch (sp->lotag) {
                ...
                case 25: // Pistons
                    ...
                    T5 = 1;
                    sector[sect].ceilingz = sp->z;
                    P_SetInterpolation(&sector[sect].ceilingz);
                    break;
                case 35:
                    sector[sect].ceilingz = sp->z;
                    break;
                ...
                case 17:
                    ...
                    if (numplayers < 2) {
                        P_SetInterpolation(&sector[sect].floorz);
                        P_SetInterpolation(&sector[sect].ceilingz);
                    }
                ...
                case 30: //No rotational subways
                    ...
                    if (sp->lotag == 0) {
                        ...
                        if (j == MAXSPRITES) {
                            sprintf(text,
                                    "Found lonely Sector Effector (lotag 0) at "
                                    "(%d,%d)\n",
                                    sp->x, sp->y);
                            gameexit(text);
                        }
                    ...
                    for (s = startwall; s < endwall; s++) {
                        ...
                        if (tempwallptr > 2047) {
                            sprintf(text,
                                    "Too many moving sectors at (%d,%d).\n",
                                    wall[s].x, wall[s].y);
                            gameexit(text);
                        }
                    }
                    if (sp->lotag == 30 || sp->lotag == 6 || sp->lotag == 14
                        || sp->lotag == 5) {
                        ...
                        if (j == 0) {
                            sprintf(text,
                                    "Subway found no zero'd sectors with "
                                    "locators\nat (%d,%d).\n",
                                    sp->x, sp->y);
                            gameexit(text);
                        }
            ...
            switch (sprite[i].lotag) {
                ...
                case 44:
                case 45:
                    B_ChangeSpriteStat(i, 15);
                    break;
                default:
                    B_ChangeSpriteStat(i, 3);
                    break;
            }
...
void animatesprites(int32_t x, int32_t y, short a, int32_t smoothratio) {
    ...
    for (j = 0; j < spritesortcnt; j++) {
        ...
        switch (t->picnum) {
            ...
            case GREENSLIME + 7:
                break;
            default:
                if (((t->cstat & 16)) || (badguy(t) && t->extra > 0)
                    || t->statnum == 10)
                    continue;
        }
    ...
    for (j = 0; j < spritesortcnt; j++) {
        ...
        if (s->statnum != 1 && s->picnum == APLAYER && ps[s->yvel].newowner == -1 && s->owner >= 0) {
            ...
            t->z += (40 << 8);
        } else if ((s->statnum == 0 && s->picnum != CRANEPOLE)
                   || s->statnum == 10 || s->statnum == 6 || s->statnum == 4
                   || s->statnum == 5 || s->statnum == 1) {
        ...
        switch (s->picnum) {
            ...
            case FORCESPHERE:
                if (t->statnum == 5) {
                    ...
                    sqa = getangle(sprite[s->owner].x - ps[screenpeek].posx,
                                   sprite[s->owner].y - ps[screenpeek].posy);
                    sqb = getangle(sprite[s->owner].x - t->x,
                                   sprite[s->owner].y - t->y);

                    if (klabs(getincangle(sqa, sqb)) > 512)
                        if (P_Dist2D(&sprite[s->owner], t) < P_Dist2D(
                                &sprite[ps[screenpeek].i], &sprite[s->owner]))
                            t->xrepeat = t->yrepeat = 0;
                }
                continue;
            case BURNING:
            case BURNING2:
                if (sprite[s->owner].statnum == 10) {
                    if (display_mirror == 0
                        && sprite[s->owner].yvel == screenpeek
                        && ps[sprite[s->owner].yvel].over_shoulder_on == 0)
                        t->xrepeat = 0;
                    else {
            ...
            case APLAYER:
                ...
                if (ps[p].over_shoulder_on > 0 && ps[p].newowner < 0) {
                    ...
                    if (screenpeek == myconnectindex && numplayers >= 2) {
                        t->x =
                            omyx
                            + mulscale16((int32_t) (myx - omyx), smoothratio);
                        t->y =
                            omyy
                            + mulscale16((int32_t) (myy - omyy), smoothratio);
                        t->z = omyz
                               + mulscale16((int32_t) (myz - omyz), smoothratio)
                               + (40 << 8);
                        t->ang = omyang
                                 + mulscale16(
                                     (int32_t) (((myang + 1024 - omyang) & 2047)
                                                - 1024),
                                     smoothratio);
                ...
                if ((display_mirror == 1 || screenpeek != p || s->owner == -1)
                    && ud.multimode > 1 && ud.showweapons
                    && sprite[ps[p].i].extra > 0 && ps[p].curr_weapon > 0) {
                    memcpy((spritetype*) &tsprite[spritesortcnt],
                           (spritetype*) t, sizeof(spritetype));
                ...
                if (ps[p].on_crane == -1
                    && (sector[s->sectnum].lotag & 0x7ff) != 1) {
                ...
                if (ud.camerasprite == -1 && ps[p].newowner == -1)
                    if (s->owner >= 0 && display_mirror == 0
                        && ps[p].over_shoulder_on == 0)
                        if (ud.multimode < 2
                            || (ud.multimode > 1 && p == screenpeek)) {
                            t->owner = -1;
                            t->xrepeat = t->yrepeat = 0;
                            continue;
                        }
            ...
            case SCRAP6 + 7:
                ...
                if (hittype[i].picnum == BLIMP && t->picnum == SCRAP1
                    && s->yvel >= 0)
                    t->picnum = s->yvel;
                else
                    t->picnum += T1;
        ...
        if (actorscrptr[s->picnum]) {
            ...
            if (t4 < 0) {
                ...
                t->picnum += k + (*CON_DecodeScript(t4)) + l * t3;
            ...
            if (t->statnum != 99 && s->picnum != EXPLOSION2 && s->picnum != HANGLIGHT && s->picnum != DOMELITE) {
                if (s->picnum != HOTMEAT) {
                    ...
                    if (ud.shadows && spritesortcnt < (MAXSPRITESONSCREEN - 2)) {
                        ...
                        if ((sector[sect].lotag & 0xff) > 2 || s->statnum == 4
                            || s->statnum == 5 || s->picnum == DRONE
                            || s->picnum == COMMANDER)
                    ...
                    if (ps[screenpeek].heat_amount > 0
                        && ps[screenpeek].heat_on) {
                        t->pal = 6;
                        t->shade = 0;
                    }
                }
        ...
        switch (s->picnum) {
            ...
            case BURNING:
            case BURNING2:
                if (sprite[s->owner].picnum != TREE1
                    && sprite[s->owner].picnum != TREE2)
                ...
                if (s->owner >= 0 && sprite[s->owner].statnum < MAXSTATUS) {
                    if (sprite[s->owner].picnum == APLAYER)
                        if (ud.camerasprite == -1)
                            if (screenpeek == sprite[s->owner].yvel
                                && display_mirror == 0) {
                                t->owner = -1;
                                break;
                            }
...
void cheats(void) {
    ...
    if (ps[myconnectindex].cheat_phase == 1) {
        while (KB_KeyWaiting()) {
            ...
            for (k = 0; k < NUMCHEATCODES; k++) {
                for (j = 0; j < cheatbuflen; j++) {
                    if (cheatbuf[j] == cheatquotes[k][j]
                        || (cheatquotes[k][j] == '#' && ch >= '0'
                            && ch <= '9')) {
            ...
            switch (k) {
                ...
                case 18: // kroz
                    ...
                    if (ud.god) { // set on
                        ...
                        HU_SetPlayerQuote(17, &ps[myconnectindex], 1);
                    } else // set off
                    {
                        ud.god = 0;
                        ...
                        HU_SetPlayerQuote(18, &ps[myconnectindex], 1);
                    }
                ...
                case 1: // stuff
                    ...
                    for (weapon = PISTOL_WEAPON; weapon < MAX_WEAPONS - j;
                         weapon++)
                        ps[myconnectindex].gotweapon[weapon] = 1;

                    for (weapon = PISTOL_WEAPON; weapon < (MAX_WEAPONS - j);
                         weapon++)
                        P_AddAmmo(weapon, &ps[myconnectindex],
                    ...
                    HU_SetPlayerQuote(5, &ps[myconnectindex], 1);
                    ps[myconnectindex].cheat_phase = 0;
                ...
                case 6:                                   // unlock
                    for (i = numsectors - 1; i >= 0; i--) //Unlock
                    {
                        ...
                        if ((j & 0x7fff) > 2) {
                            ...
                            P_OperateSectors(i, ps[myconnectindex].i);
                        }
                    }
                    P_OperateForceFields(ps[myconnectindex].i, -1);

                    HU_SetPlayerQuote(100, &ps[myconnectindex], 1);
                    ps[myconnectindex].cheat_phase = 0;
                    KB_FlushKeyboardQueue();
                    return;
                ...
                case 9: // rate
                    ud.tickrate ^= 1;
                    P_ViewChanged(); // FIX_00056: Refresh issue w/FPS, small Weapon and custom FTA, when screen resized down
                    ps[myconnectindex].cheat_phase = 0;
                    KB_FlushKeyboardQueue();
                    return;

                case 11: // beta
                    HU_SetPlayerQuote(105, &ps[myconnectindex], 1);
                ...
                case 12: // hyper
                    ...
                    ps[myconnectindex].heat_amount = 1200;
                    ps[myconnectindex].cheat_phase = 0;
                    HU_SetPlayerQuote(37, &ps[myconnectindex], 1);
                    KB_FlushKeyboardQueue();
                    return;
                ...
                case 16: // todd
                    HU_SetPlayerQuote(99, &ps[myconnectindex], 1);
                    ps[myconnectindex].cheat_phase = 0;
                    KB_FlushKeyboardQueue();
                    return;
                ...
                case 17: // showmap
                    ...
                    if (ud.showallmap) {
                        ...
                        for (i = 0; i < (MAXWALLS >> 3); i++)
                            show2dwall[i] = 255;
                        HU_SetPlayerQuote(111, &ps[myconnectindex], 1);
                    } else {
                        for (i = 0; i < (MAXSECTORS >> 3); i++)
                            show2dsector[i] = 0;
                        for (i = 0; i < (MAXWALLS >> 3); i++)
                            show2dwall[i] = 0;
                        HU_SetPlayerQuote(1, &ps[myconnectindex], 1);
                    }
                    ps[myconnectindex].cheat_phase = 0;
                    KB_FlushKeyboardQueue();
                    return;
                ...
                case 19: // allen
                    HU_SetPlayerQuote(79, &ps[myconnectindex], 1);
                    ps[myconnectindex].cheat_phase = 0;
                    KB_ClearKeyDown(sc_N);
                    return;
                ...
                case 20: // clip
                    ...
                    KB_FlushKeyboardQueue();
                    ps[myconnectindex].cheat_phase = 0;
                    HU_SetPlayerQuote(112 + ud.clipping, &ps[myconnectindex],
                                      1);
                    return;
                ...
                case 21: // weapons
                    ...
                    KB_FlushKeyboardQueue();
                    ps[myconnectindex].cheat_phase = 0;
                    HU_SetPlayerQuote(119, &ps[myconnectindex], 1);
                    return;
                ...
                case 22: // inventory
                    ...
                    ps[myconnectindex].jetpack_amount = 1600;
                    ps[myconnectindex].firstaid_amount = max_player_health;
                    HU_SetPlayerQuote(120, &ps[myconnectindex], 1);
                    ps[myconnectindex].cheat_phase = 0;
                    return;
                ...
                case 23: // keys
                    ...
                    KB_FlushKeyboardQueue();
                    ps[myconnectindex].cheat_phase = 0;
                    HU_SetPlayerQuote(121, &ps[myconnectindex], 1);
                    return;
        ...
        if (KB_KeyPressed(sc_D)) {
            if (ps[myconnectindex].cheat_phase >= 0 && numplayers < 2
                && ud.recstat == 0)
        ...
        if (KB_KeyPressed(sc_N)) {
            if (ps[myconnectindex].cheat_phase == -1) {
                if (ud.player_skill == 4) {
                    HU_SetPlayerQuote(22, &ps[myconnectindex], 1);
                    ps[myconnectindex].cheat_phase = 0;
                } else {
                    ps[myconnectindex].cheat_phase = 1;
...
void nonsharedkeys(void) {
    ...
    if (!ALT_IS_PRESSED && ud.overhead_on == 0) {
        if (ACTION(GF_ENLARGE_SCREEN)) {
            CONTROL_ClearAction(GF_ENLARGE_SCREEN);
            if (ud.screen_size > 0)
                sound(THUD);
        ...
        if (ACTION(GF_SHRINK_SCREEN)) {
            CONTROL_ClearAction(GF_SHRINK_SCREEN);
    ...
    if (ACTION(GF_SEE_COOP_VIEW) && (ud.coop == 1 || ud.recstat == 2)) {
        CONTROL_ClearAction(GF_SEE_COOP_VIEW);
    ...
    if (ud.multimode > 1 && ACTION(GF_SHOW_OPPONENTS_WEAPON)) {
        CONTROL_ClearAction(GF_SHOW_OPPONENTS_WEAPON);
        ud.showweapons = 1 - ud.showweapons;
        HU_SetPlayerQuote(82 - ud.showweapons, &ps[screenpeek], 1);
    }

    if (ACTION(GF_TOGGLE_CROSSHAIR)) {
        CONTROL_ClearAction(GF_TOGGLE_CROSSHAIR);
        ud.crosshair = 1 - ud.crosshair;
        HU_SetPlayerQuote(21 - ud.crosshair, &ps[screenpeek], 1);
    }

    if (ud.overhead_on && ACTION(GF_MAP_FOLLOW_MODE)) {
        CONTROL_ClearAction(GF_MAP_FOLLOW_MODE);
        ...
        HU_SetPlayerQuote(83 + ud.scrollmode, &ps[myconnectindex], 1);
    }
    ...
    if (SHIFTS_IS_PRESSED || ALT_IS_PRESSED) {
        ...
        if (i) {
            if (SHIFTS_IS_PRESSED) {
                if (i == 5 && ps[myconnectindex].fta > 0
                    && ps[myconnectindex].ftq == 26) {
                    music_select++;
                    ...
                    HU_SetPlayerQuote(26, &ps[myconnectindex], 1);
                    return;
                }

                HU_AddUserQuote(ud.ridecule[i - 1]);
            ...
            if (ud.lockout == 0)
                if (SoundToggle && ALT_IS_PRESSED && (RTS_NumSounds() > 0)
                    && rtsplaying == 0 && VoiceToggle) {
    ...
    if (!ALT_IS_PRESSED && !SHIFTS_IS_PRESSED) {
        ...
        if (ud.multimode > 1 && ACTION(GF_SEND_MESSAGE)) {
            KB_FlushKeyboardQueue();
            CONTROL_ClearAction(GF_SEND_MESSAGE);
        ...
        //        if(ud.multimode < 2)
        {
            ...
            FAKE_F2:
                if (sprite[ps[myconnectindex].i].extra <= 0) {
                    HU_SetPlayerQuote(118, &ps[myconnectindex], 1);
                    return;
                }
                cmenu(350);
                screencapt = 1;
        ...
        if (KB_KeyPressed(sc_F7)) {
            ...
            HU_SetPlayerQuote(109 + ps[myconnectindex].over_shoulder_on,
                              &ps[myconnectindex], 1);
        }
        ...
        if (KB_KeyPressed(sc_F8)) {
            KB_ClearKeyDown(sc_F8);
            ud.fta_on = !ud.fta_on;
            HU_SetPlayerQuote(24 - ud.fta_on, &ps[myconnectindex], 1);
        }
        ...
        if (ud.overhead_on != 0) {
            ...
            nonsharedtimer += j;
            if (ACTION(GF_ENLARGE_SCREEN))
                ps[myconnectindex].zoom +=
                    mulscale6(j, max(ps[myconnectindex].zoom, 256));
            if (ACTION(GF_SHRINK_SCREEN))
                ps[myconnectindex].zoom -=
                    mulscale6(j, max(ps[myconnectindex].zoom, 256));
    ...
    if (KB_KeyPressed(sc_Escape) && ud.overhead_on
    ...
    if (ACTION(GF_AUTORUN)) {
        CONTROL_ClearAction(GF_AUTORUN);
        ud.auto_run = 1 - ud.auto_run;
        HU_SetPlayerQuote(85 + ud.auto_run, &ps[myconnectindex], 1);
    }

    if (ACTION(GF_MAP)) {
        CONTROL_ClearAction(GF_MAP);
    ...
    if (KB_KeyPressed(sc_F11)) {
        ...
        if (SHIFTS_IS_PRESSED)
            ud.brightness -=
                8; // Keyboard step must be 8, as the brightness cursor step.
        else
            ud.brightness += 8;
        ...
        VID_SetBrightness(ud.brightness >> 2, &ps[myconnectindex].palette[0]);
        if (ud.brightness < 40)
            HU_SetPlayerQuote(29 + (ud.brightness >> 3), &ps[myconnectindex],
                              1);
        else if (ud.brightness < 80)
            HU_SetPlayerQuote(96 + (ud.brightness >> 3) - 5,
                              &ps[myconnectindex], 1);
    }
}
...
void CMD_Help(char** argv) {
    ...
    puts(" /ns           No sound");
    puts(" /nm           No music");
    puts(" /t#           Respawn, 1 = Monsters, 2 = Items, 3 = Inventory, x = "
         "All");
    puts(" /c#           MP mode, 1 = DukeMatch(spawn), 2 = Coop, 3 = "
         "Dukematch(no spawn)");
    ...
    puts(" /gFILE, /g... Use multiple group files (must be last on command "
         "line)");
...
static void CMD_ShowHelp(char** argv) {
    CMD_Help(argv);
    Error(EXIT_SUCCESS, "");
}

static void CMD_SetWeaponOrder(const char* c) {
    if (*c == '\0') {
...
static void CMD_LoadGame(const char* c) {
    ud.warp_on = 2 + (*c) - '0';
}

static void CMD_SetSkill(const char* c) {
    ud.player_skill = (int32_t) (atol(c) % 5);
...
static void CMD_SetVolume(const char* c) {
    ud.warp_on = 1;
    ud.volume_number = (int32_t) (atol(c) - 1);
    ud.m_volume_number = ud.volume_number;
}

static void CMD_ShowVersion(void) {
    Error(EXIT_SUCCESS, "This port has a auto adaptive version system. "
                        "All versions supported, simply change your "
                        "duke3d.grp\n");
}

static void CMD_PlayLevel(const char* c) {
    ud.warp_on = 1;
    ud.level_number = (atol(c) - 1) % 11;
    ud.m_level_number = ud.level_number;
}

static void CMD_PlayDemo(char* c) {
    if (strchr(c, '.') == 0) {
        strcat(c, ".dmo");
    }
    printf("Play demo %s.\n", c);
...
static void CMD_RecordDemo(void) {
    ud.m_recstat = 1;
    puts("Demo record mode on.");
}

static void CMD_UseFakeMultiplayer(const char* c) {
    puts("Fake multiplayer mode.");
    if (*(++c) == 0) {
        ud.multimode_bot = 1;
    } else {
        ud.multimode_bot = atol(c) % 17;
    }
...
static void CMD_ShowCoordinates(void) {
    ud.coords = 1;
}

static void CMD_DisableMonsters(const char* c) {
    if (*(c + 1) != 'a' && *(c + 1) != 'A') {
        ud.m_monsters_off = 1;
...
static void CMD_SetRespawnMode(const char* c) {
    switch (*c) {
        case '1':
...
static void CMD_LimitPackets(const char* c) {
    if (*c == '1') {
        movesperpacket = 1;
    }
    if (*c == '2') {
...
static void CMD_SkipMemoryCheck(void) {
    memorycheckoveride = 1;
}

static void CMD_SetMultiplayerMode(const char* c) {
    // C1 = DM C2 =COOP
    if (*c == '1' || *c == '2' || *c == '3') {
        ud.m_coop = *c - '0' - 1;
    } else {
...
static void CMD_SetNetMode(const char* c) {
    if (*c == '0') {
        networkmode = 0;
    }
    if (*c == '1') {
...
static void CMD_Disable(const char* c, char** argv) {
    if (*c == 's' || *c == 'S') {
...
static void CMD_UsePlayerAI(void) {
    ud.playerai = 1;
    puts("Other player AI.");
}

static void CMD_UseGrpFile(char* c) {
    if (*c == '\0') {
        return;
    }
    if (strchr(c, '.') == NULL) {
    ...
    char grp_file[16] = {0};
    const char* game_dir = getGameDir();
    if (game_dir[0] != '\0') {
        sprintf(grp_file, "%s\\%s", game_dir, c);
    } else {
        strcpy(grp_file, c);
    }
    const int32_t j = initgroupfile(grp_file);
    if (j == -1) {
        printf("Could not find group file %s.\n", grp_file);
    }
}

static void CMD_UseConFile(const char* c) {
    if (*c == '\0') {
        return;
    }
    const char* game_dir = getGameDir();
...
static void CMD_DisableAutoAim(void) {
    printf("\nThe Host used the /disableautoaim option to turn "
           "your Weapon AutoAim OFF\n"
           "Playing without AutoAim is usually extremely difficult "
           "and can make boring\n"
           "games ruining Duke's playability. Duke3D was not "
           "designed to play with\n"
           "AutoAim OFF like in modern FPS.\n\n"
           "Do you authorize the HOST to turn your AutoAim OFF "
           "(Y/N)? ");
...
static void CMD_Stun(void) {
    g_bStun = 1;
}

static void CMD_GameDir(const char* dir) {
    setGameDir(dir);
}
...
static void CMD_UseMap(const char* name) {
    strcpy(boardfilename, name);
    if (strchr(boardfilename, '.') == NULL) {
        strcat(boardfilename, ".map");
    }
    printf("Using level: '%s'.\n", boardfilename);
}

static void CMD_PrintArgs(int argc, char** argv) {
    printf("Commands: ");
...
void CMD_Execute(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (stricmp(c, "-game_dir") == 0) {
            i++; // Skip over the file name now that we have it.
            CMD_GameDir(argv[i]);
            continue;
        }
        if (stricmp(c, "-stun") == 0) {
        ...
        if (*c == '/') {
            ...
            switch (*c) {
                ...
                case 'T':
                    c++;
                    CMD_SetRespawnMode(c);
                    break;
                case 'm':
                case 'M':
                    CMD_DisableMonsters(c);
                    break;
                case 'w':
                case 'W':
                    CMD_ShowCoordinates();
...
void printstr(short x, short y, uint8_t string[81], uint8_t attribute) {
    ...
    while (string[i] != 0) {
        character = string[i];
        printchrasm(0xb8000 + (int32_t) pos, 1L,
                    (((int32_t) attribute << 8) + (int32_t) character));
        i++;
        pos += 2;
    }
}
...
void Logo(void) {
    ...
    if (ud.showcinematics && numplayers < 2) {
        ...
        palto(0, 0, 0, 63, false);
        rotatesprite(0, 0, 65536L, 0, DREALMS, 0, 0, 2 + 8 + 16 + 64, 0, 0,
                     xdim - 1, ydim - 1);
        nextpage();
        for (i = 63; i > 0; i -= 7)
            palto(0, 0, 0, i, true);
        ...
        rotatesprite(0, 0, 65536L, 0, BETASCREEN, 0, 0, 2 + 8 + 16 + 64, 0, 0,
                     xdim - 1, ydim - 1);
        KB_FlushKeyboardQueue();
        nextpage();
        for (i = 63; i > 0; i -= 7)
        ...
        while (totalclock < (860 + 120) && !KB_KeyWaiting()) {
            rotatesprite(0, 0, 65536L, 0, BETASCREEN, 0, 0, 2 + 8 + 16 + 64, 0,
                         0, xdim - 1, ydim - 1);
            ...
            if (totalclock > 120 && totalclock < (120 + 60)) {
                ...
                rotatesprite(160 << 16, 104 << 16, (totalclock - 120) << 10, 0,
                             DUKENUKEM, 0, 0, 2 + 8, 0, 0, xdim - 1, ydim - 1);
            } else if (totalclock >= (120 + 60))
                rotatesprite(160 << 16, (104) << 16, 60 << 10, 0, DUKENUKEM, 0,
            ...
            if (totalclock > 220 && totalclock < (220 + 30)) {
                ...
                rotatesprite(160 << 16, (104) << 16, 60 << 10, 0, DUKENUKEM, 0,
                             0, 2 + 8, 0, 0, xdim - 1, ydim - 1);
                rotatesprite(160 << 16, (129) << 16, (totalclock - 220) << 11,
                             0, THREEDEE, 0, 0, 2 + 8, 0, 0, xdim - 1,
                             ydim - 1);
            } else if (totalclock >= (220 + 30))
                rotatesprite(160 << 16, (129) << 16, 30 << 11, 0, THREEDEE, 0,
                             0, 2 + 8, 0, 0, xdim - 1, ydim - 1);
            ...
            if (PLUTOPAK) // FIX_00064: Cinematics explosions were not right for 1.3/1.3d grp.
            {
                ...
                if (totalclock >= 280 && totalclock < 395) {
                    rotatesprite(160 << 16, (151) << 16,
                                 (410 - totalclock) << 12, 0,
                                 PLUTOPAKSPRITE + 1, 0, 0, 2 + 8, 0, 0,
                                 xdim - 1, ydim - 1);
                    if (soundanm == 2) {
                        soundanm = 3;
                        sound(FLY_BY);
                    ...
                    rotatesprite(160 << 16, (151) << 16, 30 << 11, 0,
                                 PLUTOPAKSPRITE + 1, 0, 0, 2 + 8, 0, 0,
                                 xdim - 1, ydim - 1);
                }
            }
        ...
        rotatesprite(0, 0, 65536L, 0, BETASCREEN, 0, 0, 2 + 8 + 16 + 64, 0, 0,
                     xdim - 1, ydim - 1);
        rotatesprite(160 << 16, (104) << 16, 60 << 10, 0, DUKENUKEM, 0, 0,
                     2 + 8, 0, 0, xdim - 1, ydim - 1);
        rotatesprite(160 << 16, (129) << 16, 30 << 11, 0, THREEDEE, 0, 0, 2 + 8,
                     0, 0, xdim - 1, ydim - 1);
        rotatesprite(160 << 16, (151) << 16, 30 << 11, 0, PLUTOPAKSPRITE + 1, 0,
                     0, 2 + 8, 0, 0, xdim - 1, ydim - 1);

        HU_DrawText(160, 190, "WAITING FOR PLAYERS", 14, 2);
        nextpage();
    } else {
...
void compilecons(void) {
    ...
    CON_LoadFile(userconfilename, 0);
}
...
void getnames(void) {
    ...
    ud.rev[myconnectindex][0] =
        true; // always true. Used to check who we validated
    ud.rev[myconnectindex][1] = DUKE_ID;
    ud.rev[myconnectindex][2] = CLEAN_DUKE_REV_X;
    ud.rev[myconnectindex][3] = CLEAN_DUKE_REV_DOT_Y;
#endif

    memcpy(ud.groupefil_crc32[myconnectindex], groupefil_crc32,
           sizeof(groupefil_crc32));
    ud.conSize[myconnectindex] =
        ud.conSize[0]; // [0] still containing the original value
    ...
    if (numplayers > 1) {
        ...
        memcpy(buf + 1 + sizeof(groupefil_crc32), ud.conSize,
               sizeof(ud.conSize[0]));
        memcpy(buf + 1 + sizeof(groupefil_crc32) + sizeof(ud.conSize[0]),
               ud.conCRC, sizeof(ud.conCRC[0]));
        memcpy(buf + 1 + sizeof(groupefil_crc32) + sizeof(ud.conSize[0])
                   + sizeof(ud.conCRC[0]),
               ud.exeCRC, sizeof(ud.exeCRC[0]));

        for (i = connecthead; i >= 0; i = connectpoint2[i])
            if (i != myconnectindex)
                sendpacket(i, (uint8_t*) buf,
                           1 + sizeof(groupefil_crc32) + sizeof(ud.conSize[0])
                               + sizeof(ud.conCRC[0]) + sizeof(ud.exeCRC[0]));
        ...
        for (l = 0, i = connecthead; i >= 0; i = connectpoint2[i])
            if (((ud.rev[i][2] << 8) + ud.rev[i][3])
                != ((CLEAN_DUKE_REV_X << 8) + CLEAN_DUKE_REV_DOT_Y))
                l = 1;
            else
                ud.rev[i][0] = true; // means we validated this guy

        if (l) {
            printf("\n*** One or more players do not have the same xDuke "
                   "version:\n\n");
            for (l = 0, i = connecthead; i >= 0; i = connectpoint2[i])
                printf("Player [%-10s] is using Clean DukeNukem3D v%d.%d\n",
                       ud.user_name[i], ud.rev[i][2], ud.rev[i][3]);
            Error(EXIT_SUCCESS, "");
        }
#endif
        ...
        for (l = 0, i = connecthead; i >= 0; i = connectpoint2[i])
            for (j = 0; j < MAXGROUPFILES; j++) {
                if (ud.groupefil_crc32[i][j]
                        != ud.groupefil_crc32[myconnectindex][j]
                    || ud.conSize[i] != ud.conSize[myconnectindex])
                    l = 1;
            }

        if (l) {
            printf("\n*** One or more players do not have the same GRP/CON "
                   "version:\n\n");
            for (i = connecthead; i >= 0; i = connectpoint2[i]) {
                for (j = 0; j < MAXGROUPFILES && ud.groupefil_crc32[i][j];
                     j++) {
                    if (j)
                        printf(
                            "                    GRP (Add-on) : %s CRC=%X\n",
...
static const char* const baseDir = "duke3d*.grp";

#ifdef _WIN32

static const char* G_FindMainGRP(void) {
    static uint8_t groupfilefullpath[512] = {0};
    ...
    return groupfilefullpath;
}

#else

static int G_GetDirLength(const struct dirent* dir) {
#ifdef __linux__
    return _D_EXACT_NAMLEN(dir);
#else
    return dir->d_namlen;
#endif
}
...
//
// Checks if the directory begins with "duke3d"
// and has extension ".grp".
//
static int G_IsMainGRP(const struct dirent* dir) {
    const char* filename = dir->d_name;
    const int length = G_GetDirLength(dir);
    if (length < 10) {
        return false;
    }
    const char* ext = &filename[length - 4];
    return strncasecmp(filename, "duke3d", 6) == 0
           && strncasecmp(ext, ".grp", 4) == 0;
}

static const char* G_GetScanDir(void) {
    static char scan_dir[512] = {0};
    if (scan_dir[0] != '\0') {
        return scan_dir;
    }
    const char* game_dir = getGameDir();
    if (game_dir[0] == '\0') {
        ...
        strcat(scan_dir, game_dir);
    }
    if (scan_dir[strlen(scan_dir) - 1] != '/') {
        strcat(scan_dir, "/");
    }
    return scan_dir;
}
...
static const char* G_FindMainGRP(void) {
    static char grp_dir[512] = {0};
    if (grp_dir[0] != '\0') {
        return grp_dir;
    }
    const char* scan_dir = G_GetScanDir();
    printf("Scanning directory '%s' for a GRP file like '%s'.\n", scan_dir,
           baseDir);
    DIR* dir = opendir(scan_dir);
    struct dirent* dirEntry;
    while ((dirEntry = readdir(dir)) != NULL) {
        if (G_IsMainGRP(dirEntry)) {
            sprintf(grp_dir, "%s", dirEntry->d_name);
            break;
        }
    }
    FixFilePath(grp_dir);
...
static int load_duke3d_groupfile(void) {
    ...
    const char* grp_dir = G_FindMainGRP();
    if (grp_dir[0] == '\0') {
        return false;
    }
    return initgroupfile(grp_dir) != -1;
}
...
//
// Check if there is a file in some GRP archive with the given name.
//
static int G_GrpHasFile(const char* file_name) {
    const int32_t fd = kopen4load(file_name, 1);
    kclose(fd);
    return fd != -1;
}
...
//
static void G_DetectVersion(void) {
    // FIX_00022:
    // Automatically recognize the shareware grp (v1.3) +
    // full version (1.3d) + atomic (1.4/1.5 grp) and the
    // con files version (either 1.3 or 1.4) (JonoF's idea).
    if (G_GrpHasFile("DUKEDC9.MAP")) {
        grpVersion = DUKEITOUTINDC_GRP;
        return;
    }
    if (G_GrpHasFile("DUKESW.BIN")) {
        grpVersion = SHAREWARE_GRP13;
        return;
    }
    if (G_GrpHasFile("E4L11.MAP")) {
        grpVersion = ATOMIC_GRP14_15;
        return;
    }
    if (G_GrpHasFile("E3L11.MAP")) {
...
static void G_WarnGrpCorruption(void) {
    const int32_t base_crc = groupefil_crc32[0];
    printf("The content of your original BASE *.GRP is corrupted. CRC=%X\n"
           "You may run in troubles. Official GRP are:\n\n",
           base_crc);

    for (int32_t i = 0; i < MAX_KNOWN_GRP; i++) {
        const crc32_t* crc_table = &crc32lookup[i];
        const char* name = crc_table->name;
        const unsigned int crc = crc_table->crc32;
        const uint32_t size = crc_table->size;
...
static void G_CheckGrpIntegrity(void) {
    // FIX_00062: Better support and identification for GRP and CON files for 1.3/1.3d/1.4/1.5
    const int32_t base_crc = groupefil_crc32[0];
    if (CRC_VALID(base_crc)) {
        const char* crc_name = grpVersion2char_from_crc(base_crc);
...
static void G_ComputeExeCRC(const char* exe_path) {
    ud.exeCRC[0] = 0;
    const int32_t fd = open(exe_path, O_BINARY | O_RDONLY);
    if (fd == -1) {
        return;
    }
    const int32_t exe_size = filelength(fd);
    uint8_t* exe = malloc(exe_size);
    if (exe) {
        read(fd, exe, exe_size);
        ud.exeCRC[0] = crc32_update(exe, exe_size, ud.exeCRC[0]);
        free(exe);
    }
    close(fd);
}
...
uint8_t opendemoread(uint8_t which_demo) // 0 = mine
{
    ...
    if (PLUTOPAK) {
        if ((ver != BYTEVERSION && ver != BYTEVERSION_116
             && ver != BYTEVERSION_117
             && ver != BYTEVERSION_118)) // || (ud.reccnt < 512) )
        {
            printf("%s is a demo version %d. We want v. %d, %d, %d, or %d (1.5 "
                   "Atomic versions)\n",
                   fname, (int) ver, BYTEVERSION_116, BYTEVERSION_117,
                   BYTEVERSION_118, BYTEVERSION);
            kclose(recfilep);
            return 0;
        }
    } else // 1.3/1.3d style
    {
        if ((ver != BYTEVERSION && ver != BYTEVERSION_27
             && ver != BYTEVERSION_28
             && ver != BYTEVERSION_29)) // || (ud.reccnt < 512) )
        {
            printf("%s is a demo version %d. We want v. %d, %d, %d or %d "
                   "(1.3/1.3d versions)\n",
                   fname, (int) ver, BYTEVERSION_27, BYTEVERSION_28,
                   BYTEVERSION_29, BYTEVERSION);
            kclose(recfilep);
            return 0;
        }
    }
    ...
    // FIX_00062: Better support and identification for GRP and CON files for 1.3/1.3d/1.4/1.5
    if (ver == BYTEVERSION) {
        kread(recfilep, (int32_t*) groupefil_crc32_from_demo,
              sizeof(groupefil_crc32_from_demo));

        for (i = 0; i < MAXGROUPFILES; i++)
            if (groupefil_crc32_from_demo[i] != groupefil_crc32[i]) {
                for (j = 0; j <= i; j++) {
                    printf(
                        "You have GRP #%d:  %s (CRC32=%X)\n"
                        "this demo expects %s (CRC32=%X)\n",
                        j, grpVersion2char_from_crc(groupefil_crc32[j]),
                        groupefil_crc32[j],
                        grpVersion2char_from_crc(groupefil_crc32_from_demo[j]),
                        groupefil_crc32_from_demo[j]);
                }
                kclose(recfilep);
                return 0;
            }
    ...
    kread(
        recfilep, (int32*) &dummy /*ud.auto_run*/,
        sizeof(
            int32)); // not needed and would affect autorun status in duke3d.cfg when quitting duke from a demo
    ...
    // FIX_00034: Demos do not turn your run mode off anymore:
    /* ud.showweapons =  */ ud.pause_on /*= ud.auto_run */ =
        0; // makes no sense to reset those 2 value!
...
void opendemowrite(void) {
    ...
    for (i = 0; i < ud.multimode; i++) {
        fwrite((int32*) &ps[i].aim_mode, sizeof(uint8_t), 1,
               frecfilep); // seems wrong; prolly not needed anyway
        // FIX_00080: Out Of Synch in demos. Tries recovering OOS in old demos v27/28/29/116/117/118. New: v30/v119.
...
void record(void) {
    ...
    for (i = connecthead; i >= 0; i = connectpoint2[i]) {
        ...
        if (ud.reccnt >= RECSYNCBUFSIZ) {
            dfwrite(recsync, sizeof(input) * ud.multimode,
                    ud.reccnt / ud.multimode, frecfilep);
            ud.reccnt = 0;
        }
    }
}
...
RECHECK:
    ...
    while (ud.reccnt > 0 || foundemo == 0) {
        ...
        if (foundemo)
            while (totalclock >= (lockclock + TICSPERFRAME)) {
                if ((i == 0) || (i >= RECSYNCBUFSIZ)) {
                    ...
                    kdfread(recsync, sizeof(input) * ud.multimode,
                            l / ud.multimode, recfilep);
                }

                for (j = connecthead; j >= 0; j = connectpoint2[j]) {
                    copybufbyte(
                        &recsync[i],
                        &inputfifo[movefifoend[j] & (MOVEFIFOSIZ - 1)][j],
                        sizeof(input));
        ...
        if (foundemo == 0)
            ...
            j = min(max((totalclock - lockclock) * (65536 / TICSPERFRAME), 0),
                    65536);
        ...
        if (VOLUMEONE)
            if (ud.show_help == 0 && (ps[myconnectindex].gm & MODE_MENU) == 0)
                rotatesprite((320 - 50) << 16, 9 << 16, 65536L, 0, BETAVERSION,
                             0, 0, 2 + 8 + 16 + 128, 0, 0, xdim - 1, ydim - 1);

        getpackets();
        nextpage();

        if (ps[myconnectindex].gm == MODE_END
            || ps[myconnectindex].gm == MODE_GAME) {
            if (foundemo)
                kclose(recfilep);
...
void fakedomovethings(void) {
    ...
    syn = (input*) &inputfifo[fakemovefifoplc & (MOVEFIFOSIZ - 1)]
                             [myconnectindex];
    ...
    if (ud.clipping == 0
        && (sector[psect].floorpicnum == MIRROR || psect < 0
            || psect >= MAXSECTORS)) {
        myx = omyx;
        myy = omyy;
    } else {
    ...
    if ((lz & 49152) == 16384 && psectlotag == 1
        && klabs(myz - j) > PHEIGHT + (16 << 8))
        psectlotag = 0;
    ...
    if (psectlotag == 2) {
        ...
        if (myz < (fz - (i << 8))
            && (P_FloorSpace(psect) | P_CeilingSpace(psect)) == 0) //falling
        {
            if ((sb_snum & 3) == 0 && myonground
                && (sector[psect].floorstat & 2)
                && myz >= (fz - (i << 8) - (16 << 8)))
                myz = fz - (i << 8);
            else {
            ...
            if (psectlotag != 1 && psectlotag != 2 && myonground == 0
                && myzvel > (6144 >> 1))
    ...
    if (p->fist_incs || p->transporter_hold > 2 || myhardlanding
        || p->access_incs > 0 || p->knee_incs > 0
        || (p->curr_weapon == TRIPBOMB_WEAPON && p->kickback_pic > 1
            && p->kickback_pic < 4)) {
        doubvel = 0;
        myxvel = 0;
        myyvel = 0;
    ...
    if (myxvel || myyvel || syn->fvel || syn->svel) {
        ...
        if ((p->curr_weapon == KNEE_WEAPON && p->kickback_pic > 10
             && myonground)
            || (myonground && (sb_snum & 2))) {
        ...
        if (shrunk) {
            myxvel = mulscale16(myxvel, (dukefriction) - (dukefriction >> 1)
                                            + (dukefriction >> 2));
            myyvel = mulscale16(myyvel, (dukefriction) - (dukefriction >> 1)
                                            + (dukefriction >> 2));
        }
    }
...
uint8_t domovethings(void) {
        ...
        if (sync[i].bits & (1 << 17)) {
            ...
            if (multiwhat) {
                ...
                if (multiwho != myconnectindex) {
                    ...
                    HU_SetPlayerQuote(122, &ps[myconnectindex], 1);
                } else {
    ...
    for (i = connecthead; i >= 0; i = connectpoint2[i])
        copybufbyte(&inputfifo[movefifoplc & (MOVEFIFOSIZ - 1)][i], &sync[i],
                    sizeof(input));
    movefifoplc++;

    P_UpdateInterpolations();
    ...
    for (i = connecthead; i >= 0; i = connectpoint2[i]) {
        ...
        if (j < 0 && networkmode == 0)
            gameexit(
                " \nThe 'MASTER/First player' just quit the game.  "
                "All\nplayers are returned from the game. This only happens in "
    ...
    if ((numplayers >= 2) && ((movefifoplc & 7) == 7)) {
        ...
        for (i = connecthead; i >= 0; i = connectpoint2[i])
            ch += ((ps[i].posx + ps[i].posy + ps[i].posz + ps[i].ang
                    + ps[i].horiz)
                   & 255);
        syncval[myconnectindex]
               [syncvalhead[myconnectindex] & (MOVEFIFOSIZ - 1)] = ch;
    ...
    if (ud.pause_on == 0) {
        P_MoveFTA();        //ST 2
        P_MoveWeapons();    //ST 5 (must be last)
        P_MoveTransports(); //ST 9

        P_MovePlayers();    //ST 10
        P_MoveFallers();    //ST 12
        P_MoveExplosions(); //ST 4

        P_MoveActors();    //ST 1
        P_MoveEffectors(); //ST 3

        P_MoveStandables(); //ST 6
        P_DoAnimations();
        P_MoveFX(); //ST 11
    }
...
void doorders(void) {
    ...
    totalclock = 0;
    KB_FlushKeyboardQueue();
    rotatesprite(0, 0, 65536L, 0, ORDERING, 0, 0, 2 + 8 + 16 + 64, 0, 0,
                 xdim - 1, ydim - 1);
    nextpage();
    for (i = 63; i > 0; i -= 7)
        palto(0, 0, 0, i, true);
    ...
    totalclock = 0;
    KB_FlushKeyboardQueue();
    rotatesprite(0, 0, 65536L, 0, ORDERING + 1, 0, 0, 2 + 8 + 16 + 64, 0, 0,
                 xdim - 1, ydim - 1);
    nextpage();
    for (i = 63; i > 0; i -= 7)
        palto(0, 0, 0, i, true);
    ...
    totalclock = 0;
    KB_FlushKeyboardQueue();
    rotatesprite(0, 0, 65536L, 0, ORDERING + 2, 0, 0, 2 + 8 + 16 + 64, 0, 0,
                 xdim - 1, ydim - 1);
    nextpage();
    for (i = 63; i > 0; i -= 7)
        palto(0, 0, 0, i, true);
    ...
    totalclock = 0;
    KB_FlushKeyboardQueue();
    rotatesprite(0, 0, 65536L, 0, ORDERING + 3, 0, 0, 2 + 8 + 16 + 64, 0, 0,
                 xdim - 1, ydim - 1);
    nextpage();
    for (i = 63; i > 0; i -= 7)
        palto(0, 0, 0, i, true);
...
void dobonus(uint8_t bonusonly) {
    ...
    int32_t breathe[] = {
        0,   30, VICTORY1 + 1, 176, 59,           30,  60, VICTORY1 + 2,
        176, 59, 60,           90,  VICTORY1 + 1, 176, 59, 90,
        120, 0,  176,          59};

    int32_t bossmove[] = {
        0,   120, VICTORY1 + 3, 86, 59, 220, 260, VICTORY1 + 4, 86, 59,
        260, 290, VICTORY1 + 5, 86, 59, 290, 320, VICTORY1 + 6, 86, 59,
        320, 350, VICTORY1 + 7, 86, 59, 350, 380, VICTORY1 + 8, 86, 59};
    ...
    if (numplayers < 2 && ud.eog && ud.from_bonus == 0)
        switch (ud.volume_number) {
            case 0:
                if (ud.lockout == 0) {
                    R_ClearView(0L);
                    rotatesprite(0, 50 << 16, 65536L, 0, VICTORY1, 0, 0,
                                 2 + 8 + 16 + 64 + 128, 0, 0, xdim - 1,
                                 ydim - 1);
                    nextpage();
                    ...
                    while (1) {
                        ...
                        if (totalclock > 390 && totalclock < 780)
                            for (t = 0; t < 35; t += 5)
                                if (bossmove[t + 2]
                                    && (totalclock % 390) > bossmove[t]
                                    && (totalclock % 390) <= bossmove[t + 1]) {
                                    if (t == 10 && bonuscnt == 1) {
                                        sound(SHOTGUN_FIRE);
                                        sound(SQUISHED);
                                        bonuscnt++;
                                    }
                                    rotatesprite(bossmove[t + 3] << 16,
                                                 bossmove[t + 4] << 16, 65536L,
                                                 0, bossmove[t + 2], 0, 0,
                                                 2 + 8 + 16 + 64 + 128, 0, 0,
                                                 xdim - 1, ydim - 1);
                                }
                        ...
                        if (totalclock < 450 || totalclock >= 750) {
                            if (totalclock >= 750) {
                                rotatesprite(86 << 16, 59 << 16, 65536L, 0,
                                             VICTORY1 + 8, 0, 0,
                                             2 + 8 + 16 + 64 + 128, 0, 0,
                                             xdim - 1, ydim - 1);
                            ...
                            for (t = 0; t < 20; t += 5)
                                if (breathe[t + 2]
                                    && (totalclock % 120) > breathe[t]
                                    && (totalclock % 120) <= breathe[t + 1]) {
                                    if (t == 5 && bonuscnt == 0) {
                                        sound(BOSSTALKTODUKE);
                                        bonuscnt++;
                                    }
                                    rotatesprite(breathe[t + 3] << 16,
                                                 breathe[t + 4] << 16, 65536L,
                                                 0, breathe[t + 2], 0, 0,
                                                 2 + 8 + 16 + 64 + 128, 0, 0,
                                                 xdim - 1, ydim - 1);
                                }
                ...
                rotatesprite(0, 0, 65536L, 0, 3292, 0, 0, 2 + 8 + 16 + 64, 0, 0,
                             xdim - 1, ydim - 1);
                nextpage();
                for (t = 63; t > 0; t--)
                    palto(0, 0, 0, t, true);
            ...
            case 1:
                ...
                KB_FlushKeyboardQueue();
                ps[myconnectindex].palette = palette;
                rotatesprite(0, 0, 65536L, 0, 3293, 0, 0, 2 + 8 + 16 + 64, 0, 0,
                             xdim - 1, ydim - 1);
                nextpage();
                for (t = 63; t > 0; t--)
                    palto(0, 0, 0, t, true);
            ...
            case 3:
                ...
                menutext(160, 70 + 16 + 16 + 16, 0, 0,
                         "LOOK FOR A DUKE NUKEM 3D");
...
FRAGBONUS:
    ...
    if (playerswhenstarted > 1 && ud.coop != 1) {
        ...
        rotatesprite(0, 0, 65536L, 0, MENUSCREEN, 16, 0, 2 + 8 + 16 + 64, 0, 0,
                     xdim - 1, ydim - 1);
        rotatesprite(160 << 16, 34 << 16, 65536L, 0, INGAMEDUKETHREEDEE, 0, 0,
                     10, 0, 0, xdim - 1, ydim - 1);
        rotatesprite((260) << 16, 36 << 16, 65536L, 0, PLUTOPAKSPRITE + 2, 0, 0,
                     2 + 8, 0, 0, xdim - 1, ydim - 1);
        HU_DrawText(160, 58 + 2, "MULTIPLAYER TOTALS", 0, 2 + 8 + 16);
        HU_DrawText(160, 58 + 10,
                    level_names[(ud.volume_number * 11) + ud.last_level - 1], 0,
                    2 + 8 + 16);

        HU_DrawText(160, 165, "PRESS ANY KEY TO CONTINUE", 0, 2 + 8 + 16);
        ...
        for (i = 0; i < playerswhenstarted; i++) {
            sprintf(text, "%-4d", i + 1);
            HU_DrawMiniText(92 + (i * 23), 80, text, 3, 2 + 8 + 16 + 128);
        }
        ...
        for (i = 0; i < playerswhenstarted; i++) {
            ...
            HU_DrawMiniText(30, 90 + t, text, 0, 2 + 8 + 16 + 128);
            HU_DrawMiniText(38, 90 + t, ud.user_name[i], ps[i].palookup,
                            2 + 8 + 16 + 128);

            for (y = 0; y < playerswhenstarted; y++) {
                if (i == y) {
                    sprintf(text, "%-4d", ps[y].fraggedself);
                    HU_DrawMiniText(92 + (y * 23), 90 + t, text, 2,
                                    2 + 8 + 16 + 128);
                    xfragtotal -= ps[y].fraggedself;
                } else {
                    sprintf(text, "%-4d", frags[i][y]);
                    HU_DrawMiniText(92 + (y * 23), 90 + t, text, 0,
                                    2 + 8 + 16 + 128);
            ...
            HU_DrawMiniText(101 + (8 * 23), 90 + t, text, 2, 2 + 8 + 16 + 128);
        ...
        for (y = 0; y < playerswhenstarted; y++) {
            ...
            HU_DrawMiniText(92 + (y * 23), 96 + (8 * 7), text, 2,
                            2 + 8 + 16 + 128);
        }

        HU_DrawMiniText(45, 96 + (8 * 7), "DEATHS", 8, 2 + 8 + 16 + 128);
        nextpage();
    ...
    rotatesprite(0, 0, 65536L, 0, BONUSSCREEN + gfx_offset, 0, 0,
                 2 + 8 + 16 + 64 + 128, 0, 0, xdim - 1, ydim - 1);

    menutext(160, 20 - 6, 0, 0,
             &level_names[(ud.volume_number * 11) + ud.last_level - 1][0]);
    menutext(160, 36 - 6, 0, 0, "COMPLETED");

    HU_DrawText(160, 192, "PRESS ANY KEY TO CONTINUE", 16, 2 + 8 + 16);
        ...
        if (ps[myconnectindex].gm & MODE_EOL) {
            ...
            if (totalclock > (1000000000L) && totalclock < (1000000320L)) {
                switch ((totalclock >> 4) % 15) {
                    ...
                    case 4:
                    case 5:
                        rotatesprite(199 << 16, 31 << 16, 65536L, 0,
                                     BONUSSCREEN + 3 + gfx_offset, 0, 0,
                                     2 + 8 + 16 + 64 + 128, 0, 0, xdim - 1,
                                     ydim - 1);
                        break;
                    case 2:
                ...
                switch ((totalclock >> 5) & 3) {
                    ...
                    case 3:
                        rotatesprite(199 << 16, 31 << 16, 65536L, 0,
                                     BONUSSCREEN + 1 + gfx_offset, 0, 0,
                                     2 + 8 + 16 + 64 + 128, 0, 0, xdim - 1,
                                     ydim - 1);
                        break;
                    case 2:
                        rotatesprite(199 << 16, 31 << 16, 65536L, 0,
                                     BONUSSCREEN + 2 + gfx_offset, 0, 0,
                                     2 + 8 + 16 + 64 + 128, 0, 0, xdim - 1,
                                     ydim - 1);
                        break;
                }
            ...
            if (totalclock > (60 * 3)) {
                HU_DrawText(10, 59 + 9, "Your Time:", 0, 2 + 8 + 16);
                HU_DrawText(10, 69 + 9, "Par time:", 0, 2 + 8 + 16);
                HU_DrawText(10, 78 + 9, "3D Realms' Time:", 0, 2 + 8 + 16);
                if (bonuscnt == 0)
                    bonuscnt++;
                ...
                if (totalclock > (60 * 4)) {
                    ...
                    HU_DrawText((320 >> 2) + 71, 60 + 9, text, 0, 2 + 8 + 16);

                    sprintf(text, "%02d:%02d",
                            (partime[ud.volume_number * 11 + ud.last_level - 1]
                             / (26 * 60))
                                % 60,
                            (partime[ud.volume_number * 11 + ud.last_level - 1]
                             / 26)
                                % 60);
                    HU_DrawText((320 >> 2) + 71, 69 + 9, text, 0, 2 + 8 + 16);

                    sprintf(
                        text, "%02d:%02d",
                        (designertime[ud.volume_number * 11 + ud.last_level - 1]
                         / (26 * 60))
                            % 60,
                        (designertime[ud.volume_number * 11 + ud.last_level - 1]
                         / 26)
                            % 60);
                    HU_DrawText((320 >> 2) + 71, 78 + 9, text, 0, 2 + 8 + 16);
                }
            }
            if (totalclock > (60 * 6)) {
                HU_DrawText(10, 94 + 9, "Enemies Killed:", 0, 2 + 8 + 16);
                HU_DrawText(10, 99 + 4 + 9, "Enemies Left:", 0, 2 + 8 + 16);
                ...
                if (totalclock > (60 * 7)) {
                    ...
                    HU_DrawText((320 >> 2) + 70, 93 + 9, text, 0, 2 + 8 + 16);
                    if (ud.player_skill > 3) {
                        sprintf(text, "N/A");
                        HU_DrawText((320 >> 2) + 70, 99 + 4 + 9, text, 0,
                                    2 + 8 + 16);
                    } else {
                        if ((ps[myconnectindex].max_actors_killed
            ...
            if (totalclock > (60 * 9)) {
                HU_DrawText(10, 120 + 9, "Secrets Found:", 0, 2 + 8 + 16);
                HU_DrawText(10, 130 + 9, "Secrets Missed:", 0, 2 + 8 + 16);
                if (bonuscnt == 4)
                    bonuscnt++;
                ...
                if (totalclock > (60 * 10)) {
                    ...
                    HU_DrawText((320 >> 2) + 70, 120 + 9, text, 0, 2 + 8 + 16);
                    if (ps[myconnectindex].secret_rooms > 0)
                        sprintf(text, "%-3d",
                                (100 * ps[myconnectindex].secret_rooms
                                 / ps[myconnectindex].max_secret_rooms));
                    sprintf(text, "%-3d",
                            ps[myconnectindex].max_secret_rooms
                                - ps[myconnectindex].secret_rooms);
                    HU_DrawText((320 >> 2) + 70, 130 + 9, text, 0, 2 + 8 + 16);
                }
            }
...
void cameratext(short i) {
    ...
    if (!T1) {
        rotatesprite(24 << 16, 33 << 16, 65536L, 0, CAMCORNER, 0, 0, 2,
                     windowx1, windowy1, windowx2, windowy2);
        rotatesprite((320 - 26) << 16, 34 << 16, 65536L, 0, CAMCORNER + 1, 0, 0,
                     2, windowx1, windowy1, windowx2, windowy2);
        rotatesprite(22 << 16, 163 << 16, 65536L, 512, CAMCORNER + 1, 0, 0,
                     2 + 4, windowx1, windowy1, windowx2, windowy2);
        rotatesprite((310 - 10) << 16, 163 << 16, 65536L, 512, CAMCORNER + 1, 0,
                     0, 2, windowx1, windowy1, windowx2, windowy2);
        if (totalclock & 16)
            rotatesprite(46 << 16, 32 << 16, 65536L, 0, CAMLIGHT, 0, 0, 2,
                         windowx1, windowy1, windowx2, windowy2);
    } else {
        flipbits = (totalclock << 1) & 48;
        for (x = 0; x < 394; x += 64)
            for (y = 0; y < 200; y += 64)
                rotatesprite(x << 16, y << 16, 65536L, 0, STATIC, 0, 0,
                             2 + flipbits, windowx1, windowy1, windowx2,
                             windowy2);
    }
}
...
void vglass(int32_t x, int32_t y, short a, short wn, short n) {
    ...
    for (z = sector[sect].ceilingz; z < sector[sect].floorz; z += zincs)
        EGS(sect, x, y, z - (TRAND & 8191), GLASSPIECES + (z & (TRAND % 3)),
            -32, 36, 36, a + 128 - (TRAND & 255), 16 + (TRAND & 31), 0, -1, 5);
}
...
void lotsofglass(short i, short wallnum, short n) {
    ...
    if (wallnum < 0) {
        for (j = n - 1; j >= 0; j--) {
            ...
            EGS(SECT, SX, SY, SZ, GLASSPIECES + (j % 3), -32, 36, 36, a,
                32 + (TRAND & 63), 1024 - (TRAND & 1023), i, 5);
        }
        return;
    }
    ...
    for (j = n; j > 0; j--) {
        ...
        if (sect >= 0) {
            z = sector[sect].floorz
                - (TRAND
                   & (klabs(sector[sect].ceilingz - sector[sect].floorz)));
            if (z < -(32 << 8) || z > (32 << 8))
                z = SZ - (32 << 8) + (TRAND & ((64 << 8) - 1));
            a = SA - 1024;
            EGS(SECT, x1, y1, z, GLASSPIECES + (j % 3), -32, 36, 36, a,
                32 + (TRAND & 63), -(TRAND & 1023), i, 5);
        }
    }
}
...
void ceilingglass(short i, short sectnum, short n) {
    ...
    for (s = startwall; s < (endwall - 1); s++) {
        ...
        for (j = n; j > 0; j--) {
            ...
            EGS(sectnum, x1, y1, z, GLASSPIECES + (j % 3), -32, 36, 36, a,
                (TRAND & 31), 0, i, 5);
        }
    }
}
...
void lotsofcolourglass(short i, short wallnum, short n) {
    ...
    if (wallnum < 0) {
        for (j = n - 1; j >= 0; j--) {
            a = TRAND & 2047;
            k = EGS(SECT, SX, SY, SZ - (TRAND & (63 << 8)),
                    GLASSPIECES + (j % 3), -32, 36, 36, a, 32 + (TRAND & 63),
                    1024 - (TRAND & 2047), i, 5);
    ...
    for (j = n; j > 0; j--) {
        ...
        if (z < -(32 << 8) || z > (32 << 8))
            z = SZ - (32 << 8) + (TRAND & ((64 << 8) - 1));
        a = SA - 1024;
        k = EGS(SECT, x1, y1, z, GLASSPIECES + (j % 3), -32, 36, 36, a,
                32 + (TRAND & 63), -(TRAND & 2047), i, 5);
...
void takescreenshot(void) {
    ...
    sprintf(text,
            "Clean DukeNukem3D(v%d.%d) %.4d.%.2d.%.2d %.2dh%.2dm%.2ds",
            CLEAN_DUKE_REV_X, CLEAN_DUKE_REV_DOT_Y,
            tmHMS->tm_year + 1900, tmHMS->tm_mon + 1, tmHMS->tm_mday,
            tmHMS->tm_hour, tmHMS->tm_min, tmHMS->tm_sec);

    if (ud.multimode
        > 1) // if more than 1 player, we add name. Then add score if DM
    {
        text[0] = '\0';
        strcat((char*) text, " [");
        for (i = connecthead; i >= 0; i = connectpoint2[i]) {
            ...
            if (ud.m_coop == 0
                || ud.m_coop == 2) // if DM or DM No spawn. Add Score as well
            {
                strcat(text, "(");
                snprintf(score, sizeof(score), "%d",
                         ps[i].frag - ps[i].fraggedself);
                strcat(text, score);
                strcat(text, ") vs ");
            } else
                strcat(text, " vs ");
    ...
    HU_SetPlayerQuote(103, &ps[screenpeek], 1);
}
...
//
// If cmd arguments /V and /L are given.
//
static void G_Warp1(void) {
    if (numplayers > 1) {
        //if in multiplayer reset everyones weapon status.
        int c;
        ...
        for (c = connecthead; c >= 0; c = connectpoint2[c]) {
            P_ResetWeapons(
                c); //without this players would spawn with no weapon.
            P_ResetInventory(c);
        }
    }

    P_NewGame(ud.m_volume_number, ud.m_level_number, ud.m_player_skill);
    P_EnterLevel(MODE_GAME); //start game.
}

//
// If game is loaded without /V or /L cmd arguments.
//
static void G_Warp0(void) {
    if (numplayers > 1 && boardfilename[0] != 0) {
        //check if a user map is loaded and in multiplayer.
        int c;
...
//
static int G_Loop(void) {
    int32_t i;
    while (!(ps[myconnectindex].gm & MODE_END)) {
        sampletimer();
        if (ud.recstat == 2 || ud.multimode > 1
            || (ud.show_help == 0
        ...
        if (ps[myconnectindex].gm & MODE_EOL
            ...
            P_EnterLevel(ps[myconnectindex].gm);
            continue;
        }
        ...
        if ((ud.show_help == 0 && ud.multimode < 2
             && !(ps[myconnectindex].gm & MODE_MENU))
            || ud.multimode > 1 || ud.recstat == 2)
            i = min(
                max((totalclock - ototalclock) * (65536L / TICSPERFRAME), 0),
                65536);
        else
            i = 65536;
        ...
        if (VOLUMEONE)
            if (ud.show_help == 0 && show_shareware > 0
                && (ps[myconnectindex].gm & MODE_MENU) == 0)
                rotatesprite((320 - 50) << 16, 9 << 16, 65536L, 0, BETAVERSION,
                             0, 0, 2 + 8 + 16 + 128, 0, 0, xdim - 1, ydim - 1);
...
static void G_InitMemory(void) {
    totalmemory = Z_AvailHeap();
    if (memorycheckoveride) {
...
static void G_ShowStartMessage(int argc, char** argv) {
    const int major = CLEAN_DUKE_REV_X;
    const int minor = CLEAN_DUKE_REV_DOT_Y;
    printf("*** Clean DukeNukem3D v%d.%d ***\n\n", major, minor);
...
static void G_Init(int argc, char** argv) {
    ...
    G_ShowStartMessage(argc, argv);

    // FIX_00033: Fake multi and AI are now fully working
    // xduke: must be done before checkcommandline or that will prevent Fakeplayer and AI
    ud.multimode = 1;

    if (!load_duke3d_groupfile()) {
        Error(EXIT_SUCCESS,
              "Could not initialize any original BASE duke3d*.grp file\n"
    ...
    if (numplayers > 1) {
        // if multimode > 1 and numplayer == 1 => fake player mode on
    ...
    if (ud.warp_on > 1 && ud.multimode < 2) {
        ...
        ps[myconnectindex].palette = palette;
        palto(0, 0, 0, 0, false);
        rotatesprite(320 << 15, 200 << 15, 65536L, 0, LOADSCREEN, 0, 0,
