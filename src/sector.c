...
void P_DoAnimations(void)
{
	...
	for(i=animatecnt-1;i>=0;i--)
	{
        ...
        if (a == animategoal[i])
        {
            P_StopInterpolation(animateptr[i]);
...
static int32_t setanimation(short animsect,int32_t *animptr, int32_t thegoal, int32_t thevel)
{
	int32_t i, j;
...
void P_AnimateCamSprite(void)
{
    ...
    if(T1 >= 11)
    {
        ...
        else if(OW >= 0 && P_Dist3D(&sprite[ps[screenpeek].i],&sprite[i]) < 2048)
            P_XYZMirror(OW,PN);
    }
    else T1++;
}
...
uint8_t  P_ActivateWarpElevators(short s,short d) //Parm = sectoreffectornum
{
    short i, sn;
...
//    sect_error = 0;
    ...
    switch(sptr->lotag&(0xffff-49152))
    {
        ...
        case 26: //The split doors
            i = P_GetAnimationGoal(&sptr->ceilingz);
            if(i == -1) //if the door has stopped
            {
                haltsoundhack = 1;
                sptr->lotag &= 0xff00;
                sptr->lotag |= 22;
                P_OperateSectors(sn,ii);
                sptr->lotag &= 0xff00;
                sptr->lotag |= 9;
                P_OperateSectors(sn,ii);
                sptr->lotag &= 0xff00;
...
//            if(ps[sprite[ii].yvel].select_dir == 1) return;
            ...
            {
                if( P_ActivateWarpElevators(i,-1) )
                    P_ActivateWarpElevators(i,1);
                else if( P_ActivateWarpElevators(i,1) )
                    P_ActivateWarpElevators(i,-1);
                return;
            }
            else
        ...
        case 17:
            ...
            i = P_GetAnimationGoal(&sptr->floorz);
    ...
    while(i >= 0)
    {
        if(sprite[i].lotag == low)
        {
            if( sprite[i].picnum == ACTIVATORLOCKED )
            {
                ...
                {
                    if(sector[SECT].lotag&16384)
                        HU_SetPlayerQuote(4,&ps[snum],0);
                    else HU_SetPlayerQuote(8,&ps[snum],0);
                }
            }
            else
            {
...
uint8_t  P_CheckHitSwitch(short snum,int32_t w,uint8_t  switchtype)
{
    ...
    switch(picnum)
    {
        ...
        case ACCESSSWITCH2:
            if(ps[snum].access_incs == 0)
            {
                if( switchpal == 0 )
                {
                    ...
                    else HU_SetPlayerQuote(70,&ps[snum],1);
                }
                ...
                else if( switchpal == 21 )
                {
                    if( ps[snum].got_access&2 )
                        ps[snum].access_incs = 1;
                    else HU_SetPlayerQuote(71,&ps[snum],1);
                }
                ...
                else if( switchpal == 23 )
                {
                    if( ps[snum].got_access&4 )
                        ps[snum].access_incs = 1;
                    else HU_SetPlayerQuote(72,&ps[snum],1);
                }
        ...
        case POWERSWITCH2:
        case POWERSWITCH2+1:
            if( P_CheckActivatorMotion( lotag ) ) return 0;
            break;
        default:
            if( P_IsDoorWall(picnum) == 0 ) return 0;
            break;
    }
    ...
    {
        default:
            if(P_IsDoorWall(picnum) == 0) break;
        case DIPSWITCH:
        case DIPSWITCH+1:
        case TECHSWITCH:
        case TECHSWITCH+1:
        ...
        case PULLSWITCH+1:
                       ...
                       switch(sprite[x].lotag)
                       {
                           ...
                           case 25:
                               ...
                               if(hittype[x].temp_data[4])
                                   HU_SetPlayerQuote(15,&ps[snum],1);
                               else HU_SetPlayerQuote(2,&ps[snum],1);
                               break;
                           case 21:
                               HU_SetPlayerQuote(2,&ps[screenpeek],1);
                               break;
                       }
                   }
                   x = nextspritestat[x];
                ...
                P_OperateActivators(lotag,snum);
                P_OperateForceFields(ps[snum].i,lotag);
                P_OperateMasterSwitches(lotag);

                if( picnum == DIPSWITCH || picnum == DIPSWITCH+1 ||
                    picnum == ALIENSWITCH || picnum == ALIENSWITCH+1 ||
                    picnum == TECHSWITCH || picnum == TECHSWITCH+1 ) return 1;

                if( hitag == 0 && P_IsDoorWall(picnum) == 0 )
                {
                    if(switchtype == 1)
                        xyzsound(SWITCH_ON,w,sx,sy,ps[snum].posz);
                    else xyzsound(SWITCH_ON,ps[snum].i,sx,sy,ps[snum].posz);
...
void P_CheckHitWall(short spr,short dawallnum,int32_t x,int32_t y,int32_t z,short atwith)
{
   ...
   if( ( (wal->cstat&16) || wal->overpicnum == BIGFORCE ) && wal->nextsector >= 0 )
    ...
    switch(wal->picnum)
    {
            ...
            case ATM:
                wal->picnum = ATMBROKE;
                P_LotsOfMoney(&sprite[spr],1+(TRAND&7));
                spritesound(GLASS_HEAVYBREAK,spr);
                break;
...
//        case CACTUSBROKE:
        ...
        case STATUE:
            if(PN == BOTTLE10)
                P_LotsOfMoney(&sprite[i],4+(TRAND&3));
    ...
    //            j = EGS(SECT,SX,SY,SZ-(TRAND%(48<<8)),SCRAP3+(TRAND&3),-8,48,48,TRAND&2047,(TRAND&63)+64,-(TRAND&4095)-(sprite[i].zvel>>2),i,5);
      //          sprite[j].pal = 2;
        ...
        case SPACEMARINE:
            ...
            P_Shoot(i,BLOODSPLAT1);
            SA = TRAND&2047;
            P_Shoot(i,BLOODSPLAT2);
            SA = TRAND&2047;
            P_Shoot(i,BLOODSPLAT3);
            SA = TRAND&2047;
            P_Shoot(i,BLOODSPLAT4);
            SA = TRAND&2047;
            P_Shoot(i,BLOODSPLAT1);
            SA = TRAND&2047;
            P_Shoot(i,BLOODSPLAT2);
            ...
            SA = TRAND&2047;
            P_Shoot(i,BLOODSPLAT4);
            P_Guts(&sprite[i],JIBS1,1,myconnectindex);
            P_Guts(&sprite[i],JIBS2,2,myconnectindex);
            P_Guts(&sprite[i],JIBS3,3,myconnectindex);
            P_Guts(&sprite[i],JIBS4,4,myconnectindex);
            P_Guts(&sprite[i],JIBS5,1,myconnectindex);
            P_Guts(&sprite[i],JIBS3,6,myconnectindex);
            sound(SQUISHED);
            deletesprite(i);
            break;
        case CHAIR1:
...
void P_CheatKeys(short snum)
{
		...
		((sb_snum && ( sync[snum].bits&(1<<17) ) == 0) && !playing_old_demo))
    {
        ...
        if( sb_snum&(1<<15) && p->heat_amount > 0 )
        {
            ...
            HU_SetPlayerQuote(106+(!p->heat_on),p,0);
        }
            ...
            if( sb_snum&(1<<20) || sb_snum&(1<<27) || p->refresh_inventory)
        {
            ...
            switch(dainv)
            {
                case 1: HU_SetPlayerQuote(3,p,0);break;
                case 2: HU_SetPlayerQuote(90,p,0);break;
                case 3: HU_SetPlayerQuote(91,p,0);break;
                case 4: HU_SetPlayerQuote(88,p,0);break;
                case 5: HU_SetPlayerQuote(101,p,0);break;
                case 6: HU_SetPlayerQuote(89,p,0);break;
                case 7: HU_SetPlayerQuote(6,p,0);break;
            }
        }
        ...
        if( sb_snum&(1<<24) && p->newowner == -1 )
        {
            if( p->holoduke_on == -1 )
            {
                ...
                if( p->holoduke_amount > 0 )
                {
                    ...
                    SP = snum;
                    sprite[i].extra = 0;
                    HU_SetPlayerQuote(47,p,0);
                }
                else HU_SetPlayerQuote(49,p,0);
        ...
        if( sb_snum&(1<<25) && p->newowner == -1)
        {
            if( p->jetpack_amount > 0 )
            {
                ...
                else
                {
                    ...
                    HU_SetPlayerQuote(53,p,0);
                }
            }
            else HU_SetPlayerQuote(50,p,0);
        }
...
void P_CheckSectors(short snum)
{
    ...
    switch(sector[p->cursectnum].lotag)
    {
        ...
        case 32767:
            ...
            HU_SetPlayerQuote(9,p,0); // secret place found
            p->secret_rooms++;
            return;
        case -1:
            for(i=connecthead;i>=0;i=connectpoint2[i])
    ...
    else if(!p->toggle_key_flag)
    {
        ...
        if(p->newowner == -1 && neartagsprite == -1 && neartagsector == -1 && neartagwall == -1 )
            if( P_IsUnderOperator(sector[sprite[p->i].sectnum].lotag) )
        ...
        if( neartagsprite == -1 && neartagwall == -1)
            ...
            {
                oldz = P_HitSprite(p->i,&neartagsprite);
                if(oldz > 1280) neartagsprite = -1;
            }

        if(neartagsprite >= 0)
        {
            if( P_CheckHitSwitch(snum,neartagsprite,1) ) return;
        ...
        if( neartagwall >= 0 )
        {
            if( wall[neartagwall].lotag > 0 && P_IsDoorWall(wall[neartagwall].picnum) )
            {
                if(hitscanwall == neartagwall || hitscanwall == -1)
                    P_CheckHitSwitch(snum,neartagwall,0);
                return;
            }
            else if(p->newowner >= 0)
            {
        ...
        if( neartagsector >= 0 && (sector[neartagsector].lotag&16384) == 0 && P_IsNearOperator(sector[neartagsector].lotag) )
        {
            ...
            P_OperateSectors(neartagsector,p->i);
        }
        else if( (sector[sprite[p->i].sectnum].lotag&16384) == 0 )
        {
            if( P_IsUnderOperator(sector[sprite[p->i].sectnum].lotag) )
            {
                ...
                P_OperateSectors(sprite[p->i].sectnum,p->i);
            }
            else P_CheckHitSwitch(snum,neartagwall,0);
        }
    }
}
