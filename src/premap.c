...
static uint8_t getsound(uint16_t num) {
    short fp;
    int32_t l;
    ...
    if ((ud.level_number == 0 && ud.volume_number == 0
         && (num == 189 || num == 232 || num == 99 || num == 233 || num == 17))
        || (l < 12288)) {
        Sound[num].lock = 2;
    ...
    for (i = 0; i < numwalls; i++)
        if (tiles[wall[i].picnum].data == NULL) {
            ...
            if (wall[i].overpicnum >= 0
                && tiles[wall[i].overpicnum].data == NULL)
...
void P_XYZMirror(short i, short tileId) {
...
static void resetprestat(short snum, uint8_t g) {
    struct player_struct* p;
    short i;
    ...
    if (((g & MODE_EOL) != MODE_EOL && numplayers < 2)
        || (ud.coop != 1 && numplayers > 1)) {
        P_ResetWeapons(snum);
...
static void prelevel(uint8_t g) {
...
void P_NewGame(uint8_t vn, uint8_t ln, uint8_t sk) {
    ...
    if (ud.m_recstat != 2 && ud.last_level >= 0 && ud.multimode > 1
        && ud.coop != 1)
        dobonus(1);
...
static void resetpspritevars(uint8_t g) {
    short i, j, nexti; //circ;
                       //    int32_t firstx,firsty;
    spritetype* s;
    uint8_t aimmode[MAXPLAYERS];
    status_bar_t tsbar[MAXPLAYERS];

#define BOT_MAX_NAME 20
    int bot_used[BOT_MAX_NAME] = {false};
    char* bot_names[] = {
        "* ELASTI",   "* ^ZookeM^", "* DOOM",    "* DRO",      "* NOX",
        "* EXP",      "* SKIPPY",   "* BRYZIAN", "* ALI-GUN",  "* ADDFAZ",
        "* TURRICAN", "* PODA",     "* EWOLF",   "* SOULIANE", "* SPANATOR",
        "* OVERLORD", "* COWBOYUK", "* JAKS",    "* BLUEDRAG", "* MOE{GER}"};


    EGS(ps[0].cursectnum, ps[0].posx, ps[0].posy, ps[0].posz, APLAYER, 0, 0, 0,
        ps[0].ang, 0, 0, 0, 10);
    ...
    if (g & MODE_DEMO
        ...
        for (i = 0; i < ud.multimode; i++)
            memcpy(ud.wchoice[i], ud.mywchoice,
                   sizeof(ud.mywchoice)); // assuming.... :-(
    ...
    if (!(g & MODE_DEMO) && numplayers < 2 && ud.multimode > 1)
        for (i = connecthead; i >= 0; i = connectpoint2[i])
            if (i != myconnectindex) {
                ...
                bot_used[j] = true;
                ps[i].fakeplayer =
                    1 + ud.playerai; // = true if fakerplayer. (==2 if AI)
            }
...
void P_GenSpriteRemaps(void) {
    ...
    for (j = 0; j < numl; j++) {
        ...
        VID_MakePalLookup((int32_t) look_pos, (uint8_t*) tempbuf, 0, 0, 0, 1);
    }
...
static void dofrontscreens(void) {
    int32_t i, j;
    ...
    if (ud.recstat != 2) {
        ...
        if (boardfilename[0] != 0 && ud.level_number == 7
            && ud.volume_number == 0) {
            menutext(160, 90, 0, 0, "ENTERING USER MAP");
            HU_GameTextPal(160, 90 + 10, boardfilename, 14, 2);
        } else {
            menutext(160, 90, 0, 0, "ENTERING");
            menutext(160, 90 + 16 + 8, 0, 0,
                     level_names[(ud.volume_number * 11) + ud.level_number]);
        }
...
void P_EnterLevel(uint8_t g) {
    ...
    if (!VOLUMEONE) {
        ...
        if (boardfilename[0] != 0 && ud.m_level_number == 7
            ...
            if (loadboard(fulllevelfilename, &ps[0].posx, &ps[0].posy,
                          &ps[0].posz, &ps[0].ang, &ps[0].cursectnum)
                == -1) {
            ...
            sprintf(
                fulllevelfilename, "%s\\%s", getGameDir(),
                level_file_names[(ud.volume_number * 11) + ud.level_number]);
            if (!SafeFileExists(fulllevelfilename)) {
                sprintf(fulllevelfilename, "%s",
                        level_file_names[(ud.volume_number * 11)
                                         + ud.level_number]);
            }
            printf("filename=%s\n", fulllevelfilename);
            if (loadboard(fulllevelfilename, &ps[0].posx, &ps[0].posy,
                ...
                == -1) {
                sprintf(text,
                        "Internal Map %s not found! Not using the right grp "
                        "file?\n",
                        level_file_names[(ud.volume_number * 11)
                                         + ud.level_number]);
                gameexit(text);
        ...
        copybufbyte(level_file_names[(ud.volume_number * 11) + ud.level_number],
                    &levname[0], l);
        levname[l] = 0;
        ...
        if ((ud.volume_number > 1)
            || loadboard(levname, &ps[0].posx, &ps[0].posy, &ps[0].posz,
                         &ps[0].ang, &ps[0].cursectnum)
                   == -1) {
            sprintf(
                text, "Internal Map %s not found in Shareware grp pack!\n",
                level_file_names[(ud.volume_number * 11) + ud.level_number]);
            gameexit(text);
        }
    }
    ...
    if ((g & MODE_DEMO) != MODE_DEMO) // don't check for demo
        for (i = connecthead; i >= 0; i = connectpoint2[i]) {
            ...
            if (!ud.rev[i][0] && !ps[i].fakeplayer)
                Error(EXIT_SUCCESS,
                      "Your opponent [%s] is using an unknown version of xDuke."
                      "You are using v%d.%d\n",
                      ud.user_name[i], CLEAN_DUKE_REV_X,
                      CLEAN_DUKE_REV_DOT_Y);
        }
#endif
...
#ifdef CHECK_XDUKE_REV
    sprintf(fta_quotes[103], "Clean Duke3D v%d.%d",
            ud.rev[myconnectindex][2], ud.rev[myconnectindex][3]);
    HU_SetPlayerQuote(103, &ps[myconnectindex], 1);
#endif
